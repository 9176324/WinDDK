...
#else
`FORALL Msg IN Messages`
# define WPP_ANNOTATE_`Msg.Name`    __annotation(L"TMF:", WPP_GUID_WTEXT WPP_USER_MSG_GUID L" `CurrentDir` // SRC=`SourceFile.Name` MJ=`ENV MAJORCOMP` MN=`ENV MINORCOMP`", L"#typev  `Msg.Name` `Msg.MsgNo` \"`Msg.Text`\" // `Msg.Indent` `Msg.GooPairs`", L"{"`FORALL Arg IN Msg.Arguments`, L"`Arg.Name`, `Arg.MofType` -- `Arg.No`" `ENDFOR Arg`, L"}")
`ENDFOR`
#endif
#else
#if !defined(WPP_USER_MSG_GUID)
...
#else
`FORALL Msg IN Messages`
# define WPP_ANNOTATE_`Msg.Name`    __annotation(L"TMF:", WPP_GUID_TEXT WPP_USER_MSG_GUID " `CurrentDir` // SRC=`SourceFile.Name` MJ=`ENV MAJORCOMP` MN=`ENV MINORCOMP`", L"#typev  `Msg.Name` `Msg.MsgNo` \"`Msg.Text`\" // `Msg.Indent` `Msg.GooPairs`", L"{"`FORALL Arg IN Msg.Arguments`, L"`Arg.Name`, `Arg.MofType` -- `Arg.No`" `ENDFOR Arg`, L"}")
`ENDFOR`
#endif
#endif
...
static const GUID WPP_LOCAL_TraceGuids[] = { WPP_GUID_STRUCT Guid }
