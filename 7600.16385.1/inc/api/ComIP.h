...
public:
	...
	//
	template<typename _InterfacePtr> _com_ptr_t(__in __drv_constant const _InterfacePtr& p) throw(_com_error)
		: m_pInterface(NULL)
	{
		if (p) {
    ...
    //
    _com_ptr_t(__in LPSTR str) throw(_com_error)
		: m_pInterface(NULL)
    {
    ...
    // Make sure correct ctor is called
    //
    _com_ptr_t(__in LPWSTR str) throw(_com_error)
		: m_pInterface(NULL)
    {
		HRESULT hr = CreateInstance(str, NULL, CLSCTX_ALL);
	...
	//
	__drv_when(null != 0, __drv_reportError("This will issue an error if any value other than null is assigned to the object.  Please review this function call."))
	_com_ptr_t(__in int null) throw(_com_error)
		: m_pInterface(NULL)
	{
		if (null != 0) {
			_com_issue_error(E_POINTER);
	...
	//
	_com_ptr_t(__in const _com_ptr_t& cp) throw()
	...
	// Saves the interface.
	//
	_com_ptr_t(__in Interface* pInterface) throw()
	...
	// be AddRef()ed.
	//
	_com_ptr_t(__in Interface* pInterface, __in bool fAddRef) throw()
		: m_pInterface(pInterface)
	{
		if (fAddRef) {
			_AddRef();
	...
	//
	template<> _com_ptr_t(__in const _variant_t& varSrc) throw(_com_error)
		: m_pInterface(NULL)
	{
	...
	//
	explicit _com_ptr_t(__in const CLSID& clsid, __in_opt IUnknown* pOuter = NULL, __in_opt DWORD dwClsContext = CLSCTX_ALL) throw(_com_error)
		: m_pInterface(NULL)
	{
	...
	// the string.
	//
  explicit _com_ptr_t(__in LPCWSTR str, __in_opt IUnknown* pOuter = NULL, __in_opt DWORD dwClsContext = CLSCTX_ALL) throw(_com_error)
		: m_pInterface(NULL)
	{
	...
	// the string.
	//
	explicit _com_ptr_t(__in LPCSTR str, __in_opt IUnknown* pOuter = NULL, __in_opt DWORD dwClsContext = CLSCTX_ALL) throw(_com_error)
		: m_pInterface(NULL)
	{
		HRESULT hr = CreateInstance(str, pOuter, dwClsContext);
	...
	//
	template<typename _InterfacePtr> _com_ptr_t& operator=(__in_opt const _InterfacePtr& p) throw(_com_error)
	{
		HRESULT hr = _QueryInterface(p);
	...
	// Saves the interface.
	//
	_com_ptr_t& operator=(__in_opt Interface* pInterface) throw()
	{
	...
	//
	_com_ptr_t& operator=(__in_opt const _com_ptr_t& cp) throw()
	{
	...
	//
	__drv_when(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it."))
	_com_ptr_t& operator=(__in_opt int null) throw(_com_error)
	{
		if (null != 0) {
			_com_issue_error(E_POINTER);
		}
	...
	// Construct a pointer for a _variant_t object.
	//
	template<> _com_ptr_t& operator=(__in_opt const _variant_t& varSrc) throw(_com_error)
	{
		HRESULT hr = QueryStdInterfaces(varSrc);
	...
	//
	void Attach(__in Interface* pInterface) throw()
	{
		_Release();
		m_pInterface = pInterface;
	}
	...
	//
	void Attach(__in Interface* pInterface, __in bool fAddRef) throw()
	{
		_Release();
		m_pInterface = pInterface;
	...
	//
	template<typename _InterfacePtr> bool operator==(__in_opt _InterfacePtr p) throw(_com_error)
	{
	...
	//
	template<> bool operator==(__in_opt Interface* p) throw(_com_error)
	{
	...
	//
	template<> bool operator==(__in_opt _com_ptr_t& p) throw()
	{
	...
	//
	template<> bool operator==(__in_opt __drv_when(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it.")) 
	...
	// Compares 2 _com_ptr_t's
	//
	template<> bool operator!=(__in_opt _com_ptr_t& p) throw(_com_error)
	{
		return !(operator==(p));
	}
	...
	// For comparison to NULL
	//
	template<> bool operator!=(__in_opt int null) throw(_com_error)
	{
	...
	//
	__checkReturn
	HRESULT CreateInstance(__in const CLSID& rclsid, __in_opt IUnknown* pOuter = NULL, __in_opt DWORD dwClsContext = CLSCTX_ALL) throw()
	{
		HRESULT hr;
	...
	//
	__checkReturn
  HRESULT CreateInstance(__in LPCWSTR clsidString, __in_opt IUnknown* pOuter = NULL, __in_opt DWORD dwClsContext = CLSCTX_ALL) throw()
    {
		if (clsidString == NULL) {
			return E_INVALIDARG;
		}
	...
	//
	__checkReturn
	HRESULT CreateInstance(__in LPCSTR clsidStringA, __in_opt IUnknown* pOuter = NULL, __in_opt DWORD dwClsContext = CLSCTX_ALL) throw()
	{
	...
	//
	__checkReturn
	HRESULT GetActiveObject(__in const CLSID& rclsid) throw()
	{
		_Release();
	...
	//
  __checkReturn
  HRESULT GetActiveObject(__in LPCWSTR clsidString) throw()
    {
		if (clsidString == NULL) {
			return E_INVALIDARG;
		}
	...
	//
	__checkReturn
	HRESULT GetActiveObject(__in LPCSTR clsidStringA) throw()
	{
		if (clsidStringA == NULL) {
			return E_INVALIDARG;
		}
	...
	//
	template<typename _InterfaceType> __checkReturn HRESULT QueryInterface(__in const IID& iid, __in _InterfaceType*& p) throw ()
	{
		if (m_pInterface != NULL) {
...
private:
	...
	//
	template<typename _InterfacePtr> __checkReturn HRESULT _QueryInterface(__in const _InterfacePtr& p) throw()
	{
		HRESULT hr;
	...
	//
	template<typename _InterfacePtr> int _CompareUnknown(__in_opt _InterfacePtr p) throw(_com_error)
	{
		IUnknown* pu1, *pu2;
	...
	//
	__checkReturn
	HRESULT QueryStdInterfaces(__in const _variant_t& varSrc) throw()
	{
...
//
template<typename _InterfaceType> bool operator==(__in_opt __drv_when(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it."))
...
template<typename _Interface, typename _InterfacePtr> bool operator==(__in_opt _Interface* i, __in_opt _com_ptr_t<_InterfacePtr>& p) throw(_com_error)
{
	return p == i;
}

template<typename _Interface> bool operator!=(__in_opt __drv_when(null != 0, __drv_reportError("This function will issue an error if any value other than NULL is assigned to it."))
