...
BOOL APIENTRY DrvAssociateSharedSurface(
    __in SURFOBJ* psoSurf,
    __in HANDLE   hPhysicalSurface,
    __in HANDLE   hLogicalSurface,
    __in SIZEL    sizl
    );
...
LONG APIENTRY DrvSynchronizeRedirectionBitmaps(
    __in DHPDEV  dhpdev,
    __out UINT64* puiFenceID
    );

__control_entrypoint(DeviceDriver)
BOOL APIENTRY DrvAccumulateD3DDirtyRect(
    __in SURFOBJ* psoSurf,
    __in RECTL*   prclDirty
    );
...
BOOL APIENTRY DrvSetPalette(
    ...
    PALOBJ *ppalo,
    FLONG   fl,
    __range(0, WINDDI_MAXSETPALETTECOLORINDEX) ULONG   iStart,
    __range(0, WINDDI_MAXSETPALETTECOLORS - iStart) ULONG   cColors
    );
...
BOOL APIENTRY EngTransparentBlt(
    ...
    __in_data_source(USER_MODE) __out_validated(GRAPHIC) ULONG      bCalledFromBitBlt
    ); 
...
ENGAPI
BOOL APIENTRY EngUpdateDeviceSurface(__in SURFOBJ *pso, __inout CLIPOBJ **ppco);
...
INT APIENTRY EngWideCharToMultiByte(
    __in                                    UINT CodePage,
    __in_bcount_opt(BytesInWideCharString) LPWSTR WideCharString,
    __in                                    INT BytesInWideCharString,
    __out_bcount_opt(BytesInMultiByteString) LPSTR MultiByteString,
    __in                                    INT BytesInMultiByteString
    );
...
__checkReturn 
__drv_valueIs(==0) 
__drv_when(pBuffer!=0&&cjBufferSize!=0&&return==1, __drv_floatSaved) 
__drv_when(pBuffer!=0&&cjBufferSize!=0, __drv_valueIs(==1;==0)) 
__drv_when(pBuffer==0||cjBufferSize==0, __drv_valueIs(>0)) 
__forceinline
ULONG
EngSaveFloatingPointState (
    __out_bcount_opt(cjBufferSize) 
    __deref __drv_neverHold(EngFloatState) 
    __deref __drv_when(pBuffer!=0&&cjBufferSize!=0&&return==1, __drv_acquiresResource(EngFloatState)) 
    __deref __drv_when(pBuffer!=0&&cjBufferSize!=0&&return==1, __post __valid)
    VOID *pBuffer, 
...
__checkReturn 
__drv_valueIs(==1;==0) 
__drv_when(return==1, __drv_floatRestored) 
__forceinline
BOOL
EngRestoreFloatingPointState (
    __in_xcount(statesize) __deref __drv_mustHold(EngFloatState) 
    __deref __drv_releasesResource(EngFloatState) VOID *pBuffer 
    )
...
ULONG APIENTRY EngSaveFloatingPointState(
    __bcount_opt(cjBufferSize) __deref __drv_neverHold(EngFloatState) 
    __deref __drv_when(pBuffer!=0&&cjBufferSize!=0&&return==1, __drv_acquiresResource(EngFloatState)) 
    __deref __drv_when(pBuffer!=0&&cjBufferSize!=0&&return==1, __post __valid)
    VOID *pBuffer, 
...
typedef BOOL    (APIENTRY *PFN_DrvSynchronizeRedirectionBitmaps)(DHPDEV, UINT64*);
typedef BOOL    (APIENTRY *PFN_DrvAccumulateD3DDirtyRect)(SURFOBJ*, RECTL*);
typedef BOOL    (APIENTRY *PFN_DrvStartDxInterop)(SURFOBJ*, BOOL);
typedef BOOL    (APIENTRY *PFN_DrvEndDxInterop)(SURFOBJ*, BOOL, BOOL*);
