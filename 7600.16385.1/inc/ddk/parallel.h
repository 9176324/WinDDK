...
(*PPARALLEL_TRY_ALLOCATE_ROUTINE) (
	__in  PVOID   TryAllocateContext
	);

typedef
VOID
(*PPARALLEL_FREE_ROUTINE) (
	__in  PVOID   FreeContext
	);

typedef
ULONG
(*PPARALLEL_QUERY_WAITERS_ROUTINE) (
	__in  PVOID   QueryAllocsContext
	);

typedef
NTSTATUS
(*PPARALLEL_SET_CHIP_MODE) (
	__in  PVOID   SetChipContext,
	__in  UCHAR   ChipMode
	);
...
(*PPARALLEL_TRY_SELECT_ROUTINE) (
	__in  PVOID   TrySelectContext,
	__in  PVOID   TrySelectCommand
	);

typedef
NTSTATUS
(*PPARALLEL_DESELECT_ROUTINE) (
	__in  PVOID   DeselectContext,
	__in  PVOID   DeselectCommand
	);
...
(*PNEGOTIATE_IEEE_MODE) (
    __in PVOID           Context,
    __in USHORT          ModeMaskFwd,
    __in USHORT          ModeMaskRev,
    __in PARALLEL_SAFETY ModeSafety,
    __in BOOLEAN         IsForward
    );
#endif
...
(*PPARALLEL_READ) (
	__in  PVOID       Context,
	__out_bcount_part(NumBytesToRead,*NumBytesRead) PVOID       Buffer,
	__in  ULONG       NumBytesToRead,
	__out PULONG      NumBytesRead,
	__in  UCHAR       Channel
	);    
...
(*PPARALLEL_WRITE) (
	__in  PVOID       Context,
	__in_bcount(NumBytesToWrite) PVOID       Buffer,
	__in  ULONG       NumBytesToWrite,
	__out PULONG      NumBytesWritten,
	__in  UCHAR       Channel
	);
...
(*PPARALLEL_TRYSELECT_DEVICE) (
    __in  PVOID                   Context,
    __in  PARALLEL_1284_COMMAND   Command
    );
