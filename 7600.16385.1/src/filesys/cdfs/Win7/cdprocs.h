...
CdInitializeVcb (
    __in PIRP_CONTEXT IrpContext,
    __inout PVCB Vcb,
    __in __drv_in(__drv_aliasesMem) PDEVICE_OBJECT TargetDeviceObject,
    __in __drv_in(__drv_aliasesMem) PVPB Vpb,
    __in __drv_in(__drv_aliasesMem) PCDROM_TOC_LARGE CdromToc,
...
CdDeleteCcb (
    __in PIRP_CONTEXT IrpContext,
    __in __drv_freesMem( Pool ) PCCB Ccb
    );
...
VOID
CdFreePool(
    __inout_opt __drv_freesMem(__drv_deref(Pool)) __drv_out_deref( __null ) PVOID *Pool
    )
{
    if (*Pool != NULL) {
