...
#include "kbdclass.tmh"

#pragma warning(disable:4152)  // function/data pointer conversion in expression
#pragma warning(disable:4213)  // case on l-value
...

{
    ...
    KbdPrint((1,"KBDCLASS-KeyboardClassInitialize: enter"));
...

{
    ...
    if ((Globals.GrandMaster == deviceExtension) && (1 == ++Globals.Opens)) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            KbdPrint((0,
                      "KBDCLASS-KeyboardClassOpenClose: Create failed (0x%x) port device object @ 0x%p",
  ...
  --*/
{
    ...
    KbdPrint((
        3,
        "KBDCLASS-KeyboardClassReadCopyData: move bytes from 0x%p to 0x%p",
        (PCHAR) DeviceExtension->DataOut,
        destination
        ));
    ...
    KbdPrint((
        3,
        "KBDCLASS-KeyboardClassCopyReadData: new DataIn 0x%p, DataOut 0x%p",
    ...
    if (moveSize > 0) {
        ...
        if (bytesInQueue == 0) {
            ...
            *InputDataConsumed += (bytesToMove / sizeof(KEYBOARD_INPUT_DATA));
            ...
            KbdPrint((
                3,
                "KBDCLASS-KeyboardClassServiceCallback: DataIn 0x%p, DataOut 0x%p",
                deviceExtension->DataIn,
                deviceExtension->DataOut
                ));
            KbdPrint((
...
{
    NTSTATUS            status;
    ULONG               uniqueErrorValue = KEYBOARD_ERROR_VALUE_BASE;
    PDEVICE_EXTENSION   deviceExtension = NULL;
    NTSTATUS            errorCode = STATUS_SUCCESS;
    ...
    KbdPrint((1,"KBDCLASS-KbdCreateClassObject: enter"));
        ...
        if (EnableFlag) {
            ...
            if (NT_SUCCESS (status)) {
                ...
                if (NULL != Irp) {
                    ...
                    KeyboardSendIrpSynchronously (Port->TopPort, Irp, FALSE);
...
--*/
{
    ...
    status = KeyboardStart(DeviceObject, item);
    ...
    ExFreePool(item);
    IoReleaseRemoveLock(&devExt->RemoveLock, KeyboardStartWorker);
}
    ...
    case IRP_MN_START_DEVICE:
        ...
        if (item) {
            ...
            if (item->Item) {
                ...
                //
                item->Irp = Irp;
                (VOID)IoAcquireRemoveLock (&data->RemoveLock, KeyboardStartWorker);
                IoCopyCurrentIrpStackLocationToNext (Irp);
        ...
        if(fallbackToSync){
            ...
            //
            (VOID)KeyboardSendIrpSynchronously (data->TopPort,
                                                      Irp,
                                                      TRUE);
            
            worker.Item = NULL;
            worker.Irp = Irp;
            status = KeyboardStart(DeviceObject, &worker);
...

{
    ...
    UNICODE_STRING          basePortName = {0};
    UNICODE_STRING          fullPortName = {0};
    WCHAR                   basePortBuffer[NAME_MAX];
    PWCHAR                  fullClassName = NULL;
    PFILE_OBJECT            file;
...
CreateWaitWakeWorkerError:
            ...
            //
            KbdPrint ((1, "KbdClass: WARNING: Failed alloc pool -> no WW Irp"));
...
--*/
{
    ...
    default:
        ...
        //
        KdPrint(("KBDCLASS:  wait wake irp failed with %x", IoStatus->Status));
    ...
    )
{
    ...
    ItemData = (PKEYBOARD_WORK_ITEM_DATA)Context;
...
#pragma warning(disable:6014) // itemData will be freed by KeyboardToggleWaitWakeWorker, 
                              ...
                              // to KeyboardToggleWaitWakeWorker since it is already 
