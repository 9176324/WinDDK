...

{
    ...
    MouPrint((1,"MOUCLASS-MouseClassInitialize: enter"));
    ...
    //
    if (basePortName.MaximumLength < basePortName.Length + sizeof(UNICODE_NULL)){
        status = STATUS_UNSUCCESSFUL;
        ASSERT(FALSE);
        goto MouseClassInitializeExit;
    }
    basePortName.Buffer[(basePortName.Length/sizeof(WCHAR)) + 1] = L'\0';
    ...
    // Add a null terminator
    //
    if (fullPortName.MaximumLength < fullPortName.Length + sizeof(UNICODE_NULL)){
        status = STATUS_UNSUCCESSFUL;
        ASSERT(FALSE);
        goto MouseClassInitializeExit;
    }
    fullPortName.Buffer[(fullPortName.Length/sizeof(WCHAR)) + 1] = L'\0';
...
MouseClassInitializeExit:
    ...
    MouPrint((1,"MOUCLASS-MouseClassInitialize: exit, 0x%x", status));
...

{
    ...
    if ((Globals.GrandMaster == deviceExtension) && (1 == ++Globals.Opens)) {
        ...
        if (status != STATUS_SUCCESS) {
            ...
            MouPrint((0,
                      "MOUCLASS-MouseClassOpenClose: Create failed (0x%x) port device object @ 0x%p",
  ...
  --*/
{
    ...
    MouPrint((
        3,
        "MOUCLASS-MouseClassCopyReadData: move bytes from 0x%p to 0x%p",
        (PCHAR) DeviceExtension->DataOut,
        destination
        ));
    ...
    MouPrint((
        3,
        "MOUCLASS-MouseClassCopyReadData: new DataIn 0x%p, DataOut 0x%p",
    ...
    *InputDataConsumed = 0;
    ...
    if (irp) {
        ...
        *InputDataConsumed += (moveSize / sizeof(MOUSE_INPUT_DATA));
        ...
        MouPrint((
            3,
            "MOUCLASS-MouseClassServiceCallback: move bytes from 0x%p to 0x%p",
    ...
    if (moveSize > 0) {
        ...
        if (bytesInQueue == 0) {
            ...
            *InputDataConsumed += (bytesToMove / sizeof(MOUSE_INPUT_DATA));
            ...
            MouPrint((
                3,
                "MOUCLASS-MouseClassServiceCallback: DataIn 0x%p, DataOut 0x%p",
                deviceExtension->DataIn,
                deviceExtension->DataOut
                ));
            MouPrint((
...

{
    ...
    for (entry = Globals.LegacyDeviceList.Flink;
        ...
        #pragma warning(push)
        #pragma warning(disable:28107) // Data->Self is held from DriverEntry
...

{
    ...
    RtlZeroMemory(&parametersPath, sizeof(parametersPath));
    RtlZeroMemory(&defaultUnicodeName, sizeof(defaultUnicodeName));
    ...
    if (NT_SUCCESS(status)) {
        ...
        //
        if (parametersPath.MaximumLength < parametersPath.Length + sizeof(UNICODE_NULL)){
            status = STATUS_UNSUCCESSFUL;
            ASSERT(FALSE);
        } else {

            parametersPath.Buffer[(parametersPath.Length/sizeof(WCHAR)) + 1] = L'\0';
...
{
    NTSTATUS            status;
    ULONG               uniqueErrorValue = MOUSE_ERROR_VALUE_BASE;
    PDEVICE_EXTENSION   deviceExtension = NULL;
    NTSTATUS            errorCode = STATUS_SUCCESS;
    ...
    MouPrint((1,"MOUCLASS-MouCreateClassObject: enter"));
    ...
    *ClassDeviceObject = NULL;
    ...
    if (NULL == Globals.GrandMaster) {
        ...
        // overflow.
        //
        if (Globals.BaseClassName.Length > MAXULONG - (sizeof(L"\\Device\\") + sizeof(L"0") + sizeof(UNICODE_NULL))) {
            status = STATUS_UNSUCCESSFUL;
        ...
        // Add a null terminator
        //
        if (fullClassName.MaximumLength < fullClassName.Length + sizeof(UNICODE_NULL)){
            status = STATUS_UNSUCCESSFUL;
            ASSERT(FALSE);
            goto MouCreateClassObjectExit;
        }
        fullClassName.Buffer[(fullClassName.Length/sizeof(WCHAR)) + 1] = L'\0';
...

{
    ...
    *NumberPortsServiced = 0;
    ...
    //
    // Make sure BasePortName is null terminated
    //
    if (BasePortName->MaximumLength < BasePortName->Length + sizeof(UNICODE_NULL)){
        status = STATUS_UNSUCCESSFUL;
        ASSERT(FALSE);
...
typedef struct _DEVICE_START_WORKER {
    PIO_WORKITEM Item;
    PIRP Irp;
} DEVICE_START_WORKER, *PDEVICE_START_WORKER;

NTSTATUS
MouseStart(
    IN PDEVICE_OBJECT DeviceObject,
    IN PDEVICE_START_WORKER item
    )
/*++

Routine Description:

    Initializes the hardware on pnp start device
...
--*/
{
    ...
    if (NT_SUCCESS (Irp->IoStatus.Status)) {
        ...
        if (NT_SUCCESS (status) && NT_SUCCESS (Irp->IoStatus.Status)) {
            ...
            ASSERTMSG ("Get Device caps Failed!", status);
        }
...
Routine Description:
    ...
    Worker to initializes the hardware on pnp start device
...
--*/
{
    ...
    PDEVICE_START_WORKER item = (PDEVICE_START_WORKER)Context;
    PIRP Irp = item->Irp;

    devExt = (PDEVICE_EXTENSION) DeviceObject->DeviceExtension;

    status = MouseStart(DeviceObject, item);
    ...
    ExFreePool(item);
    IoReleaseRemoveLock(&devExt->RemoveLock, MouseStartWorker);
}
...
Routine Description:
    ...
    Completion routine for PnP Start IRP
...
Arguments:
    ...
    Context - Previously allocated work item object
...
--*/
{
    ...
    PPORT               port;
    PVOID               notifyHandle;
    PDEVICE_START_WORKER item;
    BOOLEAN             fallbackToSync = FALSE;
    ...
    case IRP_MN_START_DEVICE:
        ...
        //
        item = ExAllocatePool(NonPagedPool, sizeof(DEVICE_START_WORKER));
        if (item) {
            RtlZeroMemory(item, sizeof(DEVICE_START_WORKER));

            item->Item = IoAllocateWorkItem( DeviceObject );
            if (item->Item) {

                //
                // We have now preallocated everything we need to process the device start asynchronously
                //
                item->Irp = Irp;
                (VOID)IoAcquireRemoveLock (&data->RemoveLock, MouseStartWorker);
                IoCopyCurrentIrpStackLocationToNext (Irp);
                ...
                IoMarkIrpPending(Irp);
                (VOID)IoCallDriver(data->TopPort, Irp);
            }else {
                fallbackToSync = TRUE;
            }
        } else {
        ...
        if(fallbackToSync){
            DEVICE_START_WORKER worker;

            
            //
            // send the start irp down and wait for it to complete
            //
            (VOID)MouseSendIrpSynchronously (data->TopPort,
                                                      Irp,
                                                      TRUE);
            
            worker.Item = NULL;
            worker.Irp = Irp;
            status = MouseStart(DeviceObject, &worker);
...

{
    ...
    RtlZeroMemory(&fullPortName, sizeof(fullPortName));
    ...
    )
{
    ...
    if (0 < Globals.Opens) {
        ...
        ASSERTMSG ("What shall I do now?", (NULL != globalPort));
...
--*/
{
    ...
    PDEVICE_EXTENSION Port = (PDEVICE_EXTENSION)Context;
...
CreateWaitWakeWorkerError:
            ...
            //
            MouPrint((1, "MouClass: WARNING: Failed alloc pool -> no WW Irp"));
...
--*/
{
    ...
    default:
        ...
        //
        KdPrint(("MOUCLASS:  wait wake irp failed with %x", IoStatus->Status));
    ...
    )
{
    ...
    ItemData = (PMOUSE_WORK_ITEM_DATA)Context;
...
--*/
{
    ...
    ItemData = (PMOUSE_WORK_ITEM_DATA)Context;
    wwState = ItemData->WaitWakeState ? TRUE : FALSE;
    data = ItemData->Data;
...
#pragma warning(push)
#pragma warning(disable:6014) // itemData will be freed by KeyboardToggleWaitWakeWorker, 
                              // we can't however add a __drv_aliasesMem annotation 
                              // to MouseToggleWaitWakeWorker since it is already 
                              // defined as an IO_WORKITEM_ROUTINE.
NTSTATUS
MouseToggleWaitWake(
    PDEVICE_EXTENSION Data,
    ...
    )
{
    PDEVICE_EXTENSION   data;
    PMOUSE_WORK_ITEM_DATA   itemData;
    PIRP S_Irp = (PIRP)Context; // The S irp that caused us to request the power.

    UNREFERENCED_PARAMETER(MinorFunction);
    UNREFERENCED_PARAMETER(D_PowerState);
    UNREFERENCED_PARAMETER(IoStatus);
