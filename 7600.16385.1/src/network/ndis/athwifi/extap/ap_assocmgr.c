...
AmMatchRsnInfo(
    ...
    __in PRSN_IE_INFO pRsnIeInfo,
    __out PUSHORT   pStatusCode
    )
{
...
AmGetPeerInfo(
    ...
    __out PDOT11_PEER_INFO PeerInfo
    )
{
...
AmFilterUnsupportedRates(
    __in PAP_ASSOC_MGR AssocMgr,
    __in PDOT11_RATE_SET StaRateSet
    )
{
    ULONG i, j;
...
AmFillMgmtFrameHeader(
    __in PAP_ASSOC_MGR AssocMgr,
    __in PDOT11_MGMT_HEADER MgmtFrame,
    __in DOT11_MGMT_SUBTYPE MgmtSubtype,
    __in PDOT11_MAC_ADDRESS DestAddr
    )
{
    ...
    )
{
    ...
    do
    {
        ...
        *phyType = VNic11QueryCurrentPhyType(AP_GET_VNIC(AssocMgr->ApPort));
