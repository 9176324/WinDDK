...
VOID
Hvl11UnblockTimedCtxS(PHVL pHvl)
{
    HvlLock(pHvl);
    pHvl->ulStatusFlags &= ~HVL_TIMED_CTXS_BLOCKED;
    HvlUnlock(pHvl);
...
__inline
VOID
HvlSetCtxSInProgress(PHVL pHvl)
{
    ASSERT(HvlIsLocked(pHvl));
...
#endif
VOID
HvlWaitForCtxSProcessingAndLock(PHVL pHvl)
{
    HvlLock(pHvl);
