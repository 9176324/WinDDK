    ...
    )
{
    ...
    //
    // Program the MAC context on the hardware. This is only done if we are not
    // surprise removed
    //
    ...
    )
{
    ...
    //
    // Remove this MAC context from the hardware. This is only done
    // if we are not surprise removed
    //
    if (!HW_TEST_ADAPTER_STATUS(Hw, HW_ADAPTER_SURPRISE_REMOVED))
    {
...
//        NdisMoveMemory(MacContext->MacAddress, Dot11ResetRequest->dot11MacAddress, DOT11_ADDRESS_SIZE);
    ...
    {
        NdisZeroMemory(&MacContext->KeyTable[index], sizeof(HW_KEY_ENTRY));
        MacContext->KeyTable[index].Key.Valid = FALSE;
    }
    ...
    {
        NdisZeroMemory(&MacContext->PeerTable[index], sizeof(HW_PEER_NODE));
        MacContext->PeerTable[index].Valid = FALSE;
    }
    MacContext->PeerNodeCount = 0;

    // Default peer is valid
    NdisZeroMemory(&MacContext->DefaultPeer, sizeof(HW_PEER_NODE));
    ...
    MacContext->DefaultTXMgmtRate = 4;  // Default mgmt rate = 2 Mbps
    ...
    )
{
    ...
    //
    if (HwMac->Hw->PhyState.SoftwareRadioOff)
    {
        // We cannot perform the start
        HW_RELEASE_HARDWARE_LOCK(HwMac->Hw, FALSE);
        MpTrace(COMP_ASSOC, DBG_SERIOUS, ("Start BSS failed because radio is OFF\n"));
    ...
    )
{
    ...
    if (HwMac->Hw->PhyState.SoftwareRadioOff)
    {
        // We cannot perform the join
        HW_RELEASE_HARDWARE_LOCK(HwMac->Hw, FALSE);
        MpTrace(COMP_ASSOC, DBG_SERIOUS, ("Join failed because radio is OFF\n"));
        return NDIS_STATUS_DOT11_POWER_STATE_INVALID;
    }
