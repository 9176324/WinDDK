    ...
    )
{
    ...
    // Clear the key data
    NdisZeroMemory(&(HwMac->KeyTable[KeyIndex].Key), sizeof(NICKEY));
    ...
    )
{
    ...
    for (index = 0; index < HW11_PEER_TABLE_SIZE; index++)
    {
        for (defaultIndex = 0; defaultIndex < DOT11_MAX_NUM_DEFAULT_KEY; defaultIndex++)
        {
            ...
            // clear these
            HwMac->PeerTable[index].PrivateKeyTable[defaultIndex].Key.Valid = FALSE;
        }
    }
    ...
    )
{
    ...
    if (peerNode == NULL)
    {
        if (keyAdd)
        {
            ...
            //
            MpTrace(COMP_OID, DBG_NORMAL, ("Peer node table full. Unable to add new peer default key\n"));
    ...
    if (ndisStatus == NDIS_STATUS_SUCCESS) 
    {
        ...
        else 
        {
            ...
            // there is no key. So we dont invalidate the node here
            //
        }
    }
...
HwSetOperationMode(
    __in  PHW                     Hw,
    __in  ULONG                   Dot11CurrentOperationMode
    )
{
    ...
    )
{
    ...
    // Beacon rate
    BeaconMsdu->TxRate = HalGetBeaconRate(Hw->Hal, BeaconMac->OperatingPhyId);
...
HwFillRateElement(
    __in  PHW                     Hw,
    __in  PHW_MAC_CONTEXT         MacContext,
    __in_bcount(RemainingLength)  PUCHAR                  IEBuffer,
    __in  ULONG                   RemainingLength,
    __out PULONG                  IELength
    )
{
    ...
    while (TRUE) 
    {
        ...
        else if (hw->ScanContext.CancelOperation)
        {
            ...
            if (hw->ScanContext.CurrentStep == ScanStepSwitchChannel)
            {
                ...
                {
                    // Call the complete handler
                    HwScanChannelSwitchComplete(hw, NULL, &ndisStatus);
                }
