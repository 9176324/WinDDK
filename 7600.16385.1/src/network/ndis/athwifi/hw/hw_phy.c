    ...
    )
{
    ...
    {
        phyMib->OperationalRateSet.ucRateSet[index] = Dot11RateSet->ucRateSet[index];
    }

    phyMib->OperationalRateSet.uRateSetLength = Dot11RateSet->uRateSetLength;
    ...
    )
{
    ...
    do
    {
        ...
        // Set the operational rate set
        ndisStatus = HwSetOperationalRateSet(Hw, 
                        PhyId, 
                        &PhyContext->OperationalRateSet
                        );
        if (ndisStatus != NDIS_STATUS_SUCCESS)
        {
            MpTrace(COMP_MISC, DBG_SERIOUS, ("Unable to set operational rate Set. Error = 0x%08x\n",
                        ndisStatus));
            break;
        }
    ...
    )
{
    ...
    PHW_PHY_CONTEXT             newPhyContext = hw->PhyState.NewPhyContext;
