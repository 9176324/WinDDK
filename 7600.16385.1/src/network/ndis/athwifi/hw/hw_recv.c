...
HwCheckForDuplicateMPDU(
    __in  PHW                     Hw,
    __in_bcount(sizeof(DOT11_MGMT_DATA_MAC_HEADER))  PUCHAR                  PacketBuffer,
    __in  BOOLEAN                 IsGoodPacket
    )
{
    UCHAR                               index;
    ...
    for (i = HW_DEFAULT_PORT_MAC_INDEX ; i < HW_MAX_NUMBER_OF_MAC; i++)
    {
            ...
            (MP_COMPARE_MAC_ADDRESS(dataFragmentHeader->Address3, currentMacContext->DesiredBSSID)))
        {
            ...
            if (peerNode && peerNode->PrivateKeyTable[keyId].Key.Valid)
            {
                ...
                {
                    decryptStatus = HwDecryptCCMP(&decryptKey->Key,
                                        Mpdu->DataStart,
                                        Mpdu->DataLength
                                        );
        ...
        if (Mpdu->DescStatus.Decrypted == TRUE)
        {
            ...
            else
            {
                ...
                if ((decryptKey == NULL) &&
                    ...
                    (Mpdu->MulticastDestination))
                {
                    // Check for software decryption again
                    softwareDecrypted = HwRxDecrypt(Hw, Mpdu);
                    if (softwareDecrypted)
                        decryptKey = Mpdu->Key;
                }
...
HwAddRxMPDUToMSDU(
    __in  PHW_RX_MSDU                 Msdu,
    __in  PHW_RX_MPDU                 Mpdu,
    __in  PDOT11_MGMT_DATA_MAC_HEADER FragmentHeader,
    __in  UCHAR                       FragNumber
    )
{
...
HwFindPartialMSDUInReassemblyLine(
    __in  PHW                     Hw,
    __in  PDOT11_MGMT_DATA_MAC_HEADER FragmentHdr, 
    __in  USHORT                  SequenceNumber
    )
{
    PHW_RX_MSDU                 msdu;
    USHORT                      i;
