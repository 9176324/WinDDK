    ...
    )
{
    ...
    do
    {
        ...
        Msdu->TxRate = HwDetermineTxRate(macContext, peerNode, Msdu, fragmentHeader);
        if (cipherSetting == HW_TX_ENCRYPT_IF_KEY_MAPPING_KEY_AVAILABLE)
        {
            ...
            //
            Msdu->TxRate = macContext->DefaultTXMgmtRate;
        }
        ...
        if (HalGetPhyMIB(Hw->Hal, Msdu->PhyId)->PhyType == dot11_phy_type_erp)
        {
            ...
            if (macContext->CTSToSelfEnabled && (Msdu->TxRate > 22))
...
HwComputeDot11TxTime(
	__in  USHORT                  FrameLength,
	__in  ULONG                   DataRate,
	__in  BOOLEAN                 ManagementFrame,
	__in  BOOLEAN                 ShortPreamble
	)
{
	USHORT			            frameTime;
    ...
    )
{
    ...
    AckCtsTime = HwComputeDot11TxTime(sAckCtsLng / 8, Msdu->TxRate, isManagement, Msdu->UseShortPreamble);
    if (LastFragment) 
    {
            ...
            switch (nicKey->AlgoId)
            {
                ...
                case DOT11_CIPHER_ALGO_CCMP:
                    ...
                    if (doSoftwareEncryption)
                    {
                        ...
                        ndisStatus = HwEncryptCCMP(nicKey, netBuffer);
    ...
    __try
    {
        *BytesCopied = 0;
             ...
             CurrentNetBuffer = NET_BUFFER_NEXT_NB(CurrentNetBuffer))
        {
            ...
            if (CurrLength > 0)
            {
                if (CurrLength + *BytesCopied > HW11_MAX_FRAME_SIZE)
                {
