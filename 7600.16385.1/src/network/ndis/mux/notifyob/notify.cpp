    ...
    DumpBindingPath( pncbp );
     ...
     if ( dwChangeFlag & (NCN_ENABLE | NCN_REMOVE) ) {
        ...
        if ( hr == S_OK ) {
            ...
            if ( hr == S_OK ) {
                #pragma prefast(suppress:26035, "&pszwInfIdLower is a Pointer to a buffer that receives a constant null-terminated string, see http://msdn.microsoft.com/en-us/library/ms805225.aspx")
                hr = pnccLower->GetId( &pszwInfIdLower );

                if ( hr == S_OK ) {
                    #pragma prefast(suppress:26035, "&pszwInfIdUpper is a Pointer to a buffer that receives a constant null-terminated string, see http://msdn.microsoft.com/en-us/library/ms805225.aspx")
                                              ...
                                              INetCfgBindingPath* pncbp)
{
    ...
    if ( dwChangeFlag & NCN_ENABLE ) {
        ...
        if ( hr == S_OK ) {
            ...
            if ( hr == S_OK ) {
                #pragma prefast(suppress:26035, "&lpszIdLower is a pointer to a buffer that receives a constant null-terminated string, see http://msdn.microsoft.com/en-us/library/ms805225.aspx")
                hr = pnccLower->GetId( &pszwInfIdLower );

                if ( hr == S_OK ) {
                    #pragma prefast(suppress:26035, "&lpszIdUpper is a pointer to a buffer that receives a constant null-terminated string, see http://msdn.microsoft.com/en-us/library/ms805225.aspx")
                    hr = pnccUpper->GetId( &pszwInfIdUpper );
    ...
    if ( lResult == ERROR_SUCCESS ) {
        ...
        if ( dwDisp != REG_CREATED_NEW_KEY ) {
            ...
            for (dwIndex=1; lResult == ERROR_SUCCESS; ++dwIndex) {
                ...
                #pragma prefast(suppress:28197, "pMiniport is saved using m_MiniportList.Insert( pAdapter,guidAdapter ) and released using m_MiniportList.Remove in the destructor")
...
HRESULT CMuxNotify::HrAddAdapter (INetCfgComponent *pnccAdapter)
{
    ...
    if ( hr == S_OK ) {
        ...
        #pragma prefast(suppress:8197, "The instance is freed in the destructor")
        #pragma prefast(suppress:28197, "pMiniport is saved using m_AdaptersToAdd.Insert( pAdapter,guidAdapter ) and released using m_AdaptersToAdd.Remove in the destructor")
        pAdapter = new CMuxPhysicalAdapter( m_pnc,
                                            &guidAdapter );
                                   ...
                                   GUID *pguidAdapter)
{
    ...
    {
        #pragma prefast(suppress:8197, "The instance is freed in the destructor")
        #pragma prefast(suppress:28197, "pMiniport is saved using pAdapter->AddMiniport( pMiniport ) and released when pAdapter is released")
