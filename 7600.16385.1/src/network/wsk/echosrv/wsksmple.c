...
__drv_arg(SocketContext, __drv_freesMem(Mem))
VOID
WskSampleFreeSocketContext(
    __in PWSKSAMPLE_SOCKET_CONTEXT SocketContext
    );

__drv_arg(SocketOpContext, __drv_aliasesMem)
VOID
WskSampleEnqueueOp(
    __in PWSKSAMPLE_SOCKET_OP_CONTEXT SocketOpContext,
    ...
    )
{
    ...
    for(i = 0; i < WSKSAMPLE_OP_COUNT; i++) {
        __analysis_assume(socketContext == socketContext->OpContext[i].SocketContext);
