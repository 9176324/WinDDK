...
// Ensure that the trailing parts of buffer are is filled with \0, even if it is larger than passed in string.
#define _WCPYSTR_FILL_TRAIL_WITH_NULL(pd,cchDest, ps)  StringCchCopyEx ((LPWSTR)(pd), (cchDest), (LPWSTR)(ps), NULL, NULL, STRSAFE_FILL_BEHIND_NULL)


#define WCPYFIELDNAME(pField, cchDest, ps)    _WCPYSTR_FILL_TRAIL_WITH_NULL((pField), (cchDest), (ps))
...
hPrinterToPlotGPC(
                                      HANDLE  hPrinter,
    __out_ecount_opt(cchDeviceName)   LPWSTR  pwDeviceName,
...
SetDefaultDMForm(
                PPLOTDEVMODE    pPlotDM,
    __out_opt   PFORMSIZE       pCurForm
    );

VOID
SetDefaultPLOTDM(
                HANDLE          hPrinter,
    __in        PPLOTGPC        pPlotGPC,
    __in_opt    LPCWSTR         pwDeviceName,
    __in        PPLOTDEVMODE    pPlotDM,
    __out_opt   PFORMSIZE       pCurForm
    );
...
ValidateSetPLOTDM(
   ...
   __in        PPLOTGPC        pPlotGPC,
   __in_opt    LPCWSTR         pwDeviceName,
   __in        PPLOTDEVMODE    pPlotDMIn,
   __inout     PPLOTDEVMODE    pPlotDMOut,
   __out_opt   PFORMSIZE       pCurForm
    );
...
LPWSTR
str2Wstr(
    __out_ecount(cchDest) LPWSTR  pwStr,
                          size_t  cchDest,
                          LPCSTR  pbStr
    );
...
LPSTR
WStr2Str(
    __out_ecount(cchDest) LPSTR   pbStr,
                          size_t  cchDest,
                          LPCWSTR pwStr
    );
...
ReadPlotFile(
                                         HANDLE  hPlotFile,
    __out_bcount_part (cToRead, *pcRead) LPVOID  pBuf,
                                         DWORD   cToRead,
                                         LPDWORD pcRead
    );
