...
LPWSTR
GetPenDataKey(
    __out_ecount(cchBuf) LPWSTR  pwBuf,
    size_t  cchBuf,
    WORD    PenNum
    )
...
UpdateFromRegistry(
    ...
    LPBYTE      pIdxPlotData,
    DWORD       cPenData,
    PPENDATA    pPenData   // cPenData appears to have overloaded meaning. So difficult to do SAL annotation here.
    )
...
Routine Description:
    ...
    registry, if successful then it update to the pointer supplied
...
Return Value:
    ...
    return TRUE if it read successful from the registry else FALSE, for each of
...
Routine Description:
    ...
    This function saves pData to to the registry
...

{
    ...
    if (Index >= PRKI_PENDATA1) {
        ...
        if ((cPenData = (UINT)HIWORD(RegIdx)) >= MAX_PENPLOTTER_PENS) {
            ...
            PLOTERR(("SetPlotRegData: cPenData too big %ld (Max=%ld)",
                                    cPenData, MAX_PENPLOTTER_PENS));
