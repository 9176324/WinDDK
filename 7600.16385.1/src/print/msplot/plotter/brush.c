    ...
    if ((psoPattern) &&
        ...
        if (RetOk) {
            ...
            if ((iHatch >= HS_DDI_MAX)  &&
                ...
                if (
                    ! SUCCEEDED(ULongMult(psoPattern->sizlBitmap.cx, 3, &ulMemAllocSize)) ||
                    ! SUCCEEDED(ULongMult(ulMemAllocSize, psoPattern->sizlBitmap.cy, &ulMemAllocSize)) ||
                    ! SUCCEEDED(ULongAdd(Size, ulMemAllocSize, &ulMemAllocSize)) ||
                    ((Size = ulMemAllocSize) < 0)
                    )
                {
                    PLOTERR(("DrvRealizeBrush: arithmetic overflow"));
                    RetOk = FALSE;
                }
