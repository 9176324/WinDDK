...
CommonStartPDEV(
    ...
    ULONG       cjDevCaps,
    __out_bcount(cjDevCaps) ULONG       *pDevCaps,
    ULONG       cjDevInfo,
...

__control_entrypoint(DeviceDriver) BOOL
DrvEnableDriver(
          ULONG           iEngineVersion,
          ULONG           cb,
    __out DRVENABLEDATA   *pded
    )
...

__control_entrypoint(DeviceDriver) DHPDEV
DrvEnablePDEV(
                            __in DEVMODEW    *pPlotDMIn,
   __in                     PWSTR       pwszLogAddr,
                            ULONG       cPatterns,
                            __in_opt HSURF       *phsurfPatterns,
                            ULONG       cjDevCaps,
    __out_bcount(cjDevCaps) ULONG       *pDevCaps,
                            ULONG       cjDevInfo,
    __out_bcount(cjDevInfo) DEVINFO     *pDevInfo,
                            HDEV        hdev,
    __in                    PWSTR       pwszDeviceName,
                            HANDLE      hDriver
    )
...

{
    ...
    if ( NULL == pDriverInfo ||  NULL == pwszDataFile )
    {
        // do nothing
        ...
        WCPYFIELDNAME(pPDev->PlotDM.dm.dmDeviceName, CCHOF(pPDev->PlotDM.dm.dmDeviceName), pwszDeviceName);
