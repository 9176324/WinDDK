...
#define pDrvHTInfo  ((PDRVHTINFO)pPDev->pvDrvHTData)
    ...
    //
    // Are the first two tests below (checking for sign) necessary? cx + cy may not overflow if either is negative,
    // but I don't see it making sense from the definitions of cx and cy above.
    //
    if ((Size.cx < 0) || (Size.cy < 0) ||
	! SUCCEEDED(ULongAdd((ULONG)Size.cx, (ULONG)Size.cy, &ulMemAllocSize)) ||
	!(HTBmpInfo.pScanBuf = (LPBYTE)LocalAlloc(LPTR, ulMemAllocSize)))
    {
    ...
    if (PF_BYTEALIGN(PlotFlags)) {
        ...
        PLOTDBG(DBG_HTBLT, ("OutpputHTBitmap: BA Fix: Rop3=%02lx -> %02lx, Flags=%04lx",
                        (DWORD)Rop3,
