...

{
    ...
    if ((Result == CDM_RESULT_NOT_HANDLED)  &&
        ...
        //
        // Make sure buffer is big enough to allow accessing pDMOut->dmDeviceName without AV.
        // Ideally we should check for (offsetof(pDMOut, dmDeviceName)+NumBytesIn(dmDeviceName)
        // but since dmDeviceName is the first member of the strucutre, the offsetof will be
        // 0, so no use putting it here.
        //
        if ( cbIn       >= (CCHOF(pDMOut->dmDeviceName)*sizeof(WCHAR)) &&
             *pcbNeeded >= (CCHOF(pDMOut->dmDeviceName)*sizeof(WCHAR)) &&
           ...
           ) 
        {
            ...
            pPlotGPC = hPrinterToPlotGPC(hPrinter, pDMOut->dmDeviceName, CCHOF(pDMOut->dmDeviceName));
            ...
            //
            // BUG 834896: Be sure to close the printer 
            // before setting last error state, otherwise
            // ClosePrinter overwrite our SetLastError.
            //
