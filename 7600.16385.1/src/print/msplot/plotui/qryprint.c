...
DQPsprintf(
                        HINSTANCE   hInst,
    __in_ecount(cchBuf) LPWSTR      pwBuf,
                        DWORD       cchBuf,
    __out_opt           LPDWORD     pcchNeeded,
                        LPCWSTR     pwszFormat,
    ...
    )
    ...
    while (c = *pwszFormat++) {
        ...
        if (pwBuf < pwEndBuf) {
            ...
            if (pwStrData) {
                ...
                StringCchCopyNEx(pwBuf, pwEndBuf-pwBuf+1, pwStrData, (int)(pwEndBuf - pwBuf + 1), NULL, NULL, STRSAFE_FILL_BEHIND_NULL);
                pwBuf += lstrlen(pwBuf);
