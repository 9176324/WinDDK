...
VOID
ShowNumbers(
    __in_ecount(4 * Count) PCHAR       pNum,
                           PPLOTVAL    pPV,
                           WORD        Type,
                           UINT        Count,
                           UINT        NumDigits,
...

{
    ...
    case PK_FORMSRC:
        ...
        pFS = (PFORMSRC)pData;
        i   = ShowString(pFS->Name, strlen(pFS->Name));
    ...
    case PK_PENDATA:
        ...
        fprintf(stdout, ", ");
        i = ShowString(PenColorVal[pPD->ColorIdx].pValName, strlen(PenColorVal[pPD->ColorIdx].pValName));
        ShowSpaces(MaxLen + 2 - i);
        break;
    }
...
PCHAR
ParseString(
    __nullterminated  LPCSTR  pKeyword,
    __inout           PCHAR   *pLineLoc,
    __in_ecount(cBuf) PCHAR   pBuf,
                      SHORT   cBuf
    )
...
BOOL
CheckFormSrc(
    __nullterminated PCHAR       pKeyword,
                     WORD        Flags,
                     SIZEL       *pSize,
                     RECTL       *pMargin,
                     LPCSTR      pFormName,
