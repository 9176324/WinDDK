...
#pragma warning(push)
#pragma warning(disable:38034)  //__WARNING_CONCATENATED_RESOURCE_STRING
    ...
    )
{
    ...
    if (nCurPortNum < MIN_COM)
    {
        nCurPortNum = MIN_COM;
    }
    else if (nCurPortNum > MAX_COM_PORT)
    {
        nCurPortNum = MAX_COM_PORT;
    }
    ...
    //
    ZeroMemory(portUsage, sizeof(portUsage));
    ...
    for(i = MIN_COM; i <= MAX_COM_PORT; i++) {
        ...
        StringCchPrintf(szCom, ARRAYSIZE(szCom), TEXT("COM%d"), i);
        if (*current & mask) {
    ...
    IN UINT             NewComNum)
{
    ...
    dwNewComNameLen = ByteCountOf((DWORD)wcslen(szNewComName) + 1);
    ...
    buffer[0] = L'\0';
    if( SetupDiGetDeviceRegistryProperty(AdvancedData->DeviceInfoSet,
                                         AdvancedData->DeviceInfoData,
                                         SPDRP_DEVICEDESC,
                                         NULL,
                                         (PBYTE) szDeviceDesc,
                                         ...
                                         NULL))
    {
        ...
        {
            StringCchCopy(buffer, ARRAYSIZE(buffer), szDeviceDesc);
        }
    }
    else {
        //
    ...
    SetupDiSetDeviceRegistryProperty(AdvancedData->DeviceInfoSet,
                                     ...
                                     SPDRP_FRIENDLYNAME,
                                     (PBYTE) buffer,
                                     ByteCountOf((DWORD)wcslen(buffer)+1));
    ...
    )
{
   ...
   UINT  i = (UINT)CB_ERR, curComNum, newComNum = (UINT)CB_ERR;
