...
#pragma alloc_text(PAGEABLE, PscrEvtDeviceAdd)
#pragma alloc_text(PAGEABLE, PscrRegisterWithSmcLib)
#pragma alloc_text(PAGEABLE, PscrEvtDevicePrepareHardware)
#pragma alloc_text(PAGEABLE, PscrEvtDeviceD0Exit)
#pragma alloc_text(PAGEABLE, PscrEvtFileCleanup)
#pragma alloc_text(PAGEABLE, PscrGetRegistryValue)
  ...
  // setup smartcard extension - vendor attribute
   ...
   //   enter correct version of the lib
    ...
    if (vendorNameStr != NULL) {
        ...
        if (NT_SUCCESS(status) && vendorNameA.Length > 0 &&
    ...
    if (ifdTypeStr != NULL) {
        ...
        if (NT_SUCCESS(status) && ifdTypeA.Length > 0 &&
...
--*/
{
    ...
    for (i=0; i < WdfCmResourceListGetCount(ResourcesTranslated); i++) {
        ...
        case CmResourceTypePort:
            ...
            ASSERT(descriptor->u.Port.Length >= 4);
