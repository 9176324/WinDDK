...
GesnDataInterpret(
    __in  PCDROM_DEVICE_EXTENSION             DeviceExtension,
    __in  PNOTIFICATION_EVENT_STATUS_HEADER   Header,
    __out PBOOLEAN                            ResendImmediately
    );
...
#pragma alloc_text(PAGE, DeviceInitializePolling)
...
DeviceInternalSetMediaChangeState(
    ...
    __out_opt PMEDIA_CHANGE_DETECTION_STATE OldState
    )
/*++
...
--*/
{
    ...
    if (NT_SUCCESS(status))
    {
        // Format the request. This request was preformatted during device initialization
        // so this call should never fail.
    ...
    if (NT_SUCCESS(status))
    {
        ...
        {
            if (DeviceExtension->TimeOutValue == 0)
            {
                TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_MCN,
                           "McnSendTestUnitIrp: DeviceExtension->TimeOutValue "
            ...
            {
                TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_MCN,
                           "McnSendTestUnitIrp: Someone set "
                           "srb->TimeOutValue to zero?! -- resetting to %x\n",
                           DeviceExtension->TimeOutValue * 2));
                srb->TimeOutValue = DeviceExtension->TimeOutValue * 2;
            }
        }
...
--*/
{
    ...
    ASSERT(info->MediaChangeRequest != NULL);
    irp = WdfRequestWdmGetIrp(info->MediaChangeRequest);

    ASSERT(!TEST_FLAG(info->MediaChangeSrb.SrbStatus, SRB_STATUS_QUEUE_FROZEN));
...
--*/
{
    ...
    RequestSetSentTime(info->MediaChangeRequest);

    WDF_REQUEST_SEND_OPTIONS_INIT(&options,
                                  WDF_REQUEST_SEND_OPTION_SYNCHRONOUS);

    requestSent = WdfRequestSend (info->MediaChangeRequest,
    ...
    {
        TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_MCN,
                   "RequestSendMcnRequest: Failed to send a Media Change Request: %!STATUS!\n",
                   WdfRequestGetStatus(info->MediaChangeRequest)));
    }
...
VOID
DeviceResetMediaChangeCountDown(
    __in PCDROM_DEVICE_EXTENSION DeviceExtension
    )
/*++
...
Arguments:
    ...
    DeviceExtension - the device to reset the timer for
...
--*/
{
    ...
    if(info != NULL)
    {
        InterlockedExchange((PLONG)&(info->MediaChangeCountDown),
                            MEDIA_CHANGE_DEFAULT_TIME);
    }

    return;
} // end DeviceResetMediaChangeCountDown()
...
--*/
{
    ...
    mediaChangeInfo = ExAllocatePoolWithTag(NonPagedPool,
    ...
    if (NT_SUCCESS(status))
    {
        ...
        // configuration.
        mediaChangeInfo->MediaChangeCountDown = MEDIA_CHANGE_DEFAULT_TIME;
        ...
        mediaChangeInfo->MediaChangeRequestTimeInUse = 0;
        mediaChangeInfo->MediaChangeRequestLost = FALSE;
        ...
        // check the device supports GESN or not, initialize GESN structure if it supports.
        {
            ...
            TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_MCN,
                       "DeviceInitializePolling: Testing for GESN\n"));
            tempStatus = DeviceInitializeGesn(DeviceExtension);

            if (NT_SUCCESS(tempStatus))
            {
                TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_MCN,
                           "DeviceInitializePolling: GESN available for %p\n", 
                           DeviceExtension->DeviceObject));
                ASSERT(mediaChangeInfo->Gesn.Supported );
                ASSERT(mediaChangeInfo->Gesn.Buffer     != NULL);
                ASSERT(mediaChangeInfo->Gesn.BufferSize != 0);
                ASSERT(mediaChangeInfo->Gesn.EventMask  != 0);
            }
            else
            {
                TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_MCN,
                           "DeviceInitializePolling: GESN *NOT* available for %p\n", 
                           DeviceExtension->DeviceObject));
                ASSERT(!mediaChangeInfo->Gesn.Supported);
                ASSERT(mediaChangeInfo->Gesn.Buffer == NULL);
                ASSERT(mediaChangeInfo->Gesn.BufferSize == 0);
...
--*/
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        {
            RequestSetupMcnRequest(DeviceExtension, TRUE);
...
--*/
{
    ...
    if (!disabled)
    {
        ...
        status = DeviceInitializePolling(DeviceExtension, FALSE);
    }
...
DeviceEnableMediaChangeDetection(
    __in    PCDROM_DEVICE_EXTENSION DeviceExtension,
    __inout PFILE_OBJECT_CONTEXT    FileObjectContext,
...
--*/
{
    ...
    if (oldCount == 0)
    {
        ...
        // Reset the MCN timer.
        DeviceResetMediaChangeCountDown(DeviceExtension);
...
--*/
{
    ...
    ASSERT(Irp == info->MediaChangeSyncIrp);
...
--*/
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        {
            TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_MCN, 
                       "DeviceCheckMediaState: device %p has detection disabled \n",
                       deviceExtension->DeviceObject));
            status = STATUS_UNSUCCESSFUL;
        }   
    }
    ...
    if (NT_SUCCESS(status))
    {
        ...
        {
            // Media change support is active and the REQUEST is waiting. Decrement the
            ...
            countDown = InterlockedDecrement((PLONG)&(info->MediaChangeCountDown));
            if (countDown != 0)
            {
                status = STATUS_UNSUCCESSFUL;
            }
    ...
    if (NT_SUCCESS(status))
    {
        info->MediaChangeRequestTimeInUse = 0;
        info->MediaChangeRequestLost = FALSE;
