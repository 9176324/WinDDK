...
--*/
{
    ...
    else 
    {
        ...
        driverExtension->DeviceNameSuffix = 0;
...
--*/
{
    ...
    // 1. Build and assign Device Name
    {
        wideDeviceName = ExAllocatePoolWithTag(NonPagedPool,
                                               64 * sizeof(WCHAR),
                                               CDROM_TAG_STRINGS);
        ...
        // NOTE: don't increment DeviceNameSuffix yet in case of failure
        status = RtlStringCchPrintfW((NTSTRSAFE_PWSTR)wideDeviceName, 
                                     64, 
                                     L"\\Device\\CdRom%d", 
                                     driverExtension->DeviceNameSuffix);
    ...
    // 2. Register PnP&Power callbacks for any we are interested in.
    ...
    // 3. Register the EvtIoInCallerContext to deal with IOCTLs that need to stay in original context.
    WdfDeviceInitSetIoInCallerContextCallback(DeviceInit, 
                                              DeviceEvtIoInCallerContext);

    // 4. Register PreprocessCallback for IRP_MJ_FLUSH_BUFFERS and IRP_MJ_SHUTDOWN
    {
    ...
    // 5. Set attributes to create Request Context area.
    {
    ...
    // 6. Register FileObject related callbacks
    {
    ...
    // 7. Initialize device-wide attributes
    {
    ...
    // 8. Now, the device can be created.
    {
        WDF_OBJECT_ATTRIBUTES_INIT_CONTEXT_TYPE(&attributes,
    ...
    // 9. Fill up basic Device Extension settings and create a remote I/O target for the next-lower driver.
    ...
    //    I/O target on surprise removal.
    {
        ...
        deviceExtension->DeviceNumber = InterlockedIncrement((PLONG)&driverExtension->DeviceNameSuffix) - 1;
    }
    {
    ...
    // 10. Claim the device, so that port driver will only accept the commands from CDROM.SYS for this device.
    ...
    //
    // 11. Set up IO queues after device being created.
    //
    {
...
#pragma prefast(pop)
    ...
    // 12. Set the alignment requirements for the device based on the host adapter requirements.
    //
    ...
    // 13. Initialization of miscellaneous internal properties
    ...
    // 14. Initialize the stuff related to media locking
    WDF_OBJECT_ATTRIBUTES_INIT(&attributes);
    attributes.ParentObject = deviceExtension->Device;
    ...
    // 15. Initialize context structures needed for asynchronous release queue and power requests
    ...
    // 16. Register interfaces for this device.

    if (NT_SUCCESS(status))
    {
        (VOID) DeviceRegisterInterface(deviceExtension, CdRomDeviceInterface);

        (VOID) DeviceRegisterInterface(deviceExtension, MountedDeviceInterface);

        //TODO: verify - does cdrom.sys really need to support WMI?
        //IoWMIRegistrationControl(DeviceObject, WMIREG_ACTION_REGISTER);
    }

    // 17. Initialize the shutdown/flush lock
    if (NT_SUCCESS(status))
    {
        WDF_OBJECT_ATTRIBUTES_INIT(&attributes);
        attributes.ParentObject = deviceExtension->Device;
    ...
    // 18. Initialize the work item that is used to initiate asynchronous reads/writes
    if (NT_SUCCESS(status))
    {
    ...
    // 19. Initialize the work item that is used to process most IOCTLs at PASSIVE_LEVEL.
    if (NT_SUCCESS(status))
    {
        WDF_WORKITEM_CONFIG workItemConfig;
        WDF_OBJECT_ATTRIBUTES workItemAttributes;
...
--*/
{
    ...
    //TODO: verify - does cdrom.sys really need to support WMI?
    // status = IoWMIRegistrationControl(deviceExtension->DeviceObject, WMIREG_ACTION_DEREGISTER);
...
--*/
{
    ...
    // Make certain not to start the timer before properly initialized
...
--*/
{
    ...
    // SYNC CACHE command should be sent to drive if the media is currently writable.
    if (NT_SUCCESS(status) &&
        deviceExtension->DeviceAdditionalData.Mmc.WriteAllowed)
    {
        // issue SYNC CACHE command.
        deviceExtension->PowerContext.PowerChangeState.PowerDown = PowerDownDeviceLocked;
        status = DeviceSendPowerProcessRequest(deviceExtension);
    }

    if (NT_SUCCESS(status) &&
        !TEST_FLAG(deviceExtension->ScanForSpecialFlags, CDROM_SPECIAL_DISABLE_SPIN_DOWN)) 
    {
        // issue STOP UNIT command.
        deviceExtension->PowerContext.PowerChangeState.PowerDown = PowerDownDeviceFlushed;
        status = DeviceSendPowerProcessRequest(deviceExtension);
        
        // status will be ignored for STOP UNIT command.
...
--*/
{
    ...
    // Close the IoTarget so that there will be no more requests sent to the next-lower driver.
...
DeviceCleanupProtectedLocks(
    ...
    __in PFILE_OBJECT_CONTEXT     FileObjectContext
    )
/*++
...
VOID
NormalizeIoctl(
    __inout PWDF_REQUEST_PARAMETERS  requestParameters
    )
{
    ULONG ioctlCode;
    ULONG baseCode;
...
--*/
{
    ...
    if (!WdfRequestSend(Request, DeviceExtension->IoTarget, NULL))
    {
        // fail the request
        RequestCompletion(DeviceExtension, Request, WdfRequestGetStatus(Request), WdfRequestGetInformation(Request));
    }
...
--*/
{
    ...
    case IOCTL_MCN_SYNC_FAKE_IOCTL:
    {
        // This is the request to process Media Change Detection.
        ...
        while (shouldRetry)
        {
            ...
            RequestSetupMcnRequest(DeviceExtension,
                                   info->Gesn.Supported);
...
--*/
{
    ...
    if (requestContext->SyncRequired)
    {
        ...
        ASSERT(KeReadStateEvent(requestContext->SyncEvent) != 0);
    }
}
