...
typedef struct _CDROM_DRIVER_EXTENSION {
    ...
    ULONG               DeviceNameSuffix;   // count of cdrom devices have been created since booted.
    ULONG               Flags;
...
typedef struct _CDROM_MMC_EXTENSION {
    ...
    __field_bcount(CapabilitiesBufferSize) 
    ...
    )
{
    ...
    ASSERT(Srb->SenseInfoBuffer != DeviceExtension->SenseData);
...
DeviceRetrieveDescriptor(
    ...
    __deref_out_opt PSTORAGE_DESCRIPTOR_HEADER* Descriptor
    );
...
RequestSenseInfoInterpret(
    __in      PCDROM_DEVICE_EXTENSION   DeviceExtension,
    __in      WDFREQUEST                Request, // get IRP MJ code, IoControlCode from it (or attached original request)
    __in      PSCSI_REQUEST_BLOCK       Srb,
    __in      ULONG                     RetriedCount,
...
DeviceScanForSpecial(
    __in PCDROM_DEVICE_EXTENSION          DeviceExtension,
    __in CDROM_SCAN_FOR_SPECIAL_INFO      DeviceList[],
    __in PCDROM_SCAN_FOR_SPECIAL_HANDLER  Function
    );
