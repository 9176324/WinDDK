...
DeviceRetrieveScsiAddress(
    __in PCDROM_DEVICE_EXTENSION DeviceExtension,
    __in PSCSI_ADDRESS           ScsiAddress
    );
...
--*/
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (deviceExtension->DeviceAdditionalData.Mmc.IsWriter)
        {
            ...
            deviceExtension->TimeOutValue = max(deviceExtension->TimeOutValue, 260);
        }
    }

    // 8. Finalish the initialization, enable the main timer
    if (NT_SUCCESS(status))
    {
...
--*/
{
    ...
    if (DeviceExtension->PrivateFdoData == NULL) 
    {
        DeviceExtension->PrivateFdoData = ExAllocatePoolWithTag(NonPagedPool,
...
--*/
{
    ...
    *Descriptor = NULL;
    ...
    ASSERT(bufferLength >= sizeof(STORAGE_PROPERTY_QUERY));
    bufferLength = max(bufferLength, sizeof(STORAGE_PROPERTY_QUERY));

    descriptor = ExAllocatePoolWithTag(NonPagedPool, bufferLength, CDROM_TAG_DESCRIPTOR);
...
--*/
{
    ...
    //   0xffffffc0 -- Currently reserved, may be used later.
    //
...
--*/
{
    ...
    {
        ASSERT(DeviceExtension->DeviceAdditionalData.CachedInquiryDataByteCount != 0);
        return STATUS_SUCCESS;
    }
