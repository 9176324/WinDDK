...
--*/
{
    ...
    WDF_REQUEST_SEND_OPTIONS_INIT(&sendOptions, sendOptionsFlags);
    ...
    __analysis_assume(sendOptions.Timeout != 0);

    if (WdfRequestSend(Request, deviceExtension->IoTarget, &sendOptions))
    {
        if (syncRequired)
        {
        ...
        {
            // this is an asynchronous send, do nothing.
    ...
    else
    {
        ...
        if (!NT_SUCCESS(status))
        {
            TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_IOCTL,
                        "RequestHandleUnknownIoctl: WdfRequestSend failed. %!STATUS!\n",
                        status));
        }
...
--*/
{
    ...
    *DataLength = 0;
    ...
    if (RequestParameters.Parameters.DeviceIoControl.InputBufferLength < sizeof(CDROM_SPEED_REQUEST)) 
    {
        status = STATUS_INFO_LENGTH_MISMATCH;
    }
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            else if (infoFound) 
            {
                ...
                ASSERT(DuidExactMatch == CompareStorageDuids((PSTORAGE_DEVICE_UNIQUE_IDENTIFIER)descHeader,
    ...
    *DataLength = 0;
    ...
    if (NT_SUCCESS(status))
    {
        if (writeSpeedRequest->RequestType == CdromPerformanceRequest)
        {
            ...
            else if (performanceRequest->Exceptions != CdromReadPerformance &&
...
--*/
{
    ...
    ASSERT(requestContext->SyncRequired);
...
--*/
{
    ...
    *DataLength = 0;
    ...
    ASSERT(DeviceExtension->DeviceName.Buffer);
...
GetConfigurationDataConversionTypeAllToTypeOne(
    __in    FEATURE_NUMBER       RequestedFeature,
    ...
    if (NT_SUCCESS(status))
    {
        ...
        else
        {
            ...
            if (sectorCount != 0)
            {
                ...
                timeoutValue = ((sectorCount + 128) / 128) * DeviceExtension->TimeOutValue;
            } 
...
--*/
{
    ...
    *DataLength = 0;
    ...
    if (NT_SUCCESS(status))
    {
        ...
        status = ScratchBuffer_ExecuteCdbEx(DeviceExtension, Request, transferSize, FALSE, &cdb, 6, DeviceExtension->TimeOutValue * 2);
...
--*/
{
    ...
    *DataLength = 0;
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (NT_SUCCESS(status))
        {
            status = ScratchBuffer_ExecuteCdb(DeviceExtension, Request, (ULONG)transferSize, TRUE, &cdb, sizeof(cdb.GET_PERFORMANCE));
        }
    ...
    *DataLength = 0;
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (currentIrpStack->Parameters.DeviceIoControl.OutputBufferLength > 0)
        {
            ...
            {
                ASSERT(!((ULONG_PTR)outputVirtAddr & DeviceExtension->AdapterDescriptor->AlignmentMask));
                *DataLength = 0;
                status = STATUS_INVALID_PARAMETER;
            }
        }
...
--*/
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        {
            ULONG tempLength = (DeviceExtension->ScratchContext.ScratchSrb->DataTransferLength > FIELD_OFFSET(DVD_DESCRIPTOR_HEADER, Data))
