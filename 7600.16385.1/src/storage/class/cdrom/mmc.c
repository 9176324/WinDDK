    ...
    *BytesReturned = 0;
    ...
    if (NT_SUCCESS(status)) 
    {
        ...
        for (i = 0; (i < 4) && NT_SUCCESS(status); i++) 
        {
            ...
            {
                // allocate the memory
                buffer = (PGET_CONFIGURATION_HEADER)ExAllocatePoolWithTag(NonPagedPoolCacheAligned,
                                                                          size,
                                                                          CDROM_TAG_FEATURE);
...
--*/
{
    ...
    status = ScratchBuffer_ExecuteCdb(DeviceExtension, NULL, transferSize, TRUE, &cdb, 12);
