...
--*/
{
    ...
    ASSERT(DeviceExtension->ScratchContext.ScratchInUse == 0);
...
--*/
{
    ...
    ASSERT(DeviceExtension->DeviceAdditionalData.MaxPageAlignedTransferBytes != 0);

    // validate no partially-saved state
    ASSERT(DeviceExtension->ScratchContext.ScratchBuffer     == NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchBufferMdl  == NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchBufferSize == 0);
    ASSERT(DeviceExtension->ScratchContext.ScratchRequest    == NULL);
    ASSERT(DeviceExtension->ScratchContext.PartialMdl == NULL);
    ...
    {
        DeviceExtension->ScratchContext.ScratchBuffer = ExAllocatePoolWithTag(NonPagedPool,
    ...
    if (NT_SUCCESS(status))
    {
        DeviceExtension->ScratchContext.ScratchSrb = ExAllocatePoolWithTag(NonPagedPool,
    ...
    if (NT_SUCCESS(status))
    {
        DeviceExtension->ScratchContext.ScratchSense = ExAllocatePoolWithTag(NonPagedPool,
                                                                             sizeof(SENSE_DATA),
                                                                             CDROM_TAG_SCRATCH);
    ...
    if (NT_SUCCESS(status))
    {
        ...
        DeviceExtension->ScratchContext.ScratchHistory = ExAllocatePoolWithTag(NonPagedPool,
                                                                               allocationSize,
                                                                               CDROM_TAG_SCRATCH);
        if (DeviceExtension->ScratchContext.ScratchHistory == NULL)
        {
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            {
                ASSERT(DeviceExtension->ScratchContext.PartialMdl->Size >= 
...
--*/
{
    ...
    ASSERT(DeviceExtension->ScratchContext.ScratchHistory    != NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchSense      != NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchSrb        != NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchRequest    != NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchBufferSize != 0);
    ASSERT(DeviceExtension->ScratchContext.ScratchBuffer     != NULL);
    ASSERT(DeviceExtension->ScratchContext.ScratchBufferMdl  != NULL);
...
--*/
{
    ...
    WDFREQUEST                          originalRequest = (WDFREQUEST) DeferredContext;
    PCDROM_REQUEST_CONTEXT              requestContext = RequestGetContext(originalRequest);
    NTSTATUS                            status = STATUS_SUCCESS;
    KIRQL                               oldIrql;
...
--*/
{
    ...
    if (!requestCancelled)
    {
        RequestSetSentTime(DeviceExtension->ScratchContext.ScratchRequest);

        WDF_REQUEST_SEND_OPTIONS_INIT(&options, SynchronousSrb ? WDF_REQUEST_SEND_OPTION_SYNCHRONOUS : 0);

        // Send the request down the stack and wait for it to complete, if it's a synchronous operation
        requestSent = WdfRequestSend(DeviceExtension->ScratchContext.ScratchRequest, DeviceExtension->IoTarget, &options);
        if (!requestSent)
        {
