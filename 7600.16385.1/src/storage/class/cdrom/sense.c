...
SenseInfoInterpretRefineByIoControl(
    ...
    __in      BOOLEAN                   OverrideVerifyVolume,
...
SenseInfoRequestGetInformation(
    ...
    __out UCHAR*      MajorFunctionCode,
    __out ULONG*      IoControlCode,
    __out BOOLEAN*    OverrideVerifyVolume,
    __out ULONGLONG*  Total100nsSinceFirstSend
    )
{
...
SenseInfoInterpretByAdditionalSenseCode(
    ...
    __in      UCHAR                     AdditionalSenseCodeQual,
    __inout   NTSTATUS*                 Status,
    __inout   BOOLEAN*                  Retry,
    __out __range(0,100) ULONG*         RetryIntervalInSeconds,  
    __inout   PERROR_LOG_CONTEXT        LogContext
    )
/*
    ...
    )
{
    ...
    *RetryIntervalInSeconds = 0;
    ...
    case SRB_STATUS_SELECTION_TIMEOUT: 
    {
        ...
        *Retry = FALSE;
        *RetryIntervalInSeconds = SECONDS_TO_100NS_UNITS(2);
        LogContext->LogError = TRUE;
...
SenseInfoInterpretRefineByScsiCommand(
    ...
    __in      PSCSI_REQUEST_BLOCK       Srb,
    __in      ULONG                     RetriedCount,
    __in      LONGLONG                  Total100nsSinceFirstSend,
    __in      BOOLEAN                   OverrideVerifyVolume,
    __inout   BOOLEAN*                  Retry,
    __inout   NTSTATUS*                 Status,
...
--*/
{
    ...
    else if (IS_SCSIOP_READWRITE(opCode))
    {
        ...
        if (*Retry)
        {
            ...
            //
            // TODO: On write speed changes (SPT or otherwise), snoop to determine new write speed.
