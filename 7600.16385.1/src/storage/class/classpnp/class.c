...
RetryRequest:
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (match == FALSE) {            
            ...
            {
                ASSERT(!"Drive capacity has changed while the device wasn't started!");
            } else {
                if (InterlockedCompareExchange(&fdoData->UpdateDiskPropertiesWorkItemActive, 1, 0))
                {
                    ASSERT(!"Read capacity recursion detected!");
                } else {
                    ...
                    if (workItem) {
                        ...
                        InterlockedExchange(&fdoData->UpdateDiskPropertiesWorkItemActive, 0);
                    }               
                }
            }
        }
...
ClassCreateDeviceObject(
    ...
    __in PDEVICE_OBJECT       LowerDevice,
    __in BOOLEAN              IsFdo,
    __out
    __drv_out_deref(
    ...
    )
{
    ...
    __analysis_assume(driverExtension != NULL);
...
BOOLEAN
ClassMarkChildMissing(
    __in PPHYSICAL_DEVICE_EXTENSION Child,
    ...
    )
{
    ...
    deviceRelations->Objects[--i] = nextChild->DeviceObject;
        ...
        status = ObReferenceObjectByPointer(
                    ...
                    KernelMode);
        if (!NT_SUCCESS(status)) {
            ASSERT(!"Error referencing child device by pointer");
    ...
    ASSERTMSG("Child list has changed: ", i == 0);
    ...
    )
{
    ...
    if (RemoveType == IRP_MN_REMOVE_DEVICE){
        ...
        (VOID)KeWaitForSingleObject(&commonExtension->RemoveEvent,
                                    Executive,
                                    KernelMode,
                                    FALSE,
                                    NULL);
    ...
    )
{
    ...
    ASSERT(FdoExtension->PrivateFdoData != NULL);

    stackSize = FdoExtension->CommonExtension.LowerDeviceObject->StackSize + 1;

    FdoExtension->PrivateFdoData->PowerProcessIrp = ExAllocatePoolWithTag(NonPagedPool,
