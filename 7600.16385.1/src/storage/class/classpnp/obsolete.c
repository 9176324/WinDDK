 ...
 */
VOID ClassSplitRequest(__in PDEVICE_OBJECT Fdo, __in PIRP Irp, __in ULONG MaximumBytes)
{
    ...
    )
{
    ...
    if (currentIrpStack->MajorFunction == IRP_MJ_READ ||
        ...
        __analysis_assume(currentIrpStack->Parameters.Read.Length <= Srb->DataTransferLength);
	...
	__analysis_assume(Irp->MdlAddress->ByteCount <= Srb->DataTransferLength);
...
VOID ClassDeleteSrbLookasideList(__inout PCOMMON_DEVICE_EXTENSION CommonExtension)
{
    PAGED_CODE();
