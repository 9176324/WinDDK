...
VOID EnqueueFreeTransferPacket(PDEVICE_OBJECT Fdo, PTRANSFER_PACKET Pkt)
{
...
#else
    InterlockedIncrement(&fdoData->NumFreeTransferPackets);
#endif
     ...
     *  Free down to LocalMaxWorkingSetTransferPackets immediately, and
    ...
    if (fdoData->NumFreeTransferPackets >= fdoData->NumTotalTransferPackets){
        ...
        if (fdoData->NumTotalTransferPackets > fdoData->LocalMaxWorkingSetTransferPackets){
            ...
            TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW, "Exiting stress, block freeing (%d-%d) packets.", fdoData->NumTotalTransferPackets, fdoData->LocalMaxWorkingSetTransferPackets));
