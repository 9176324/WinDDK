...
typedef struct _DSM_DEVICE_INFO {
    ...
    //
    // This indicates whether this device has been reserved.
    //
    BOOLEAN IsReserved;

    //
    // This flag is set when the PR key is successfully registered down this device-path pair.
    //
    ...
    //
    // Flag to indicate that RPTG has already been sent down in Inquire, so
...
typedef struct _DSM_GROUP_ENTRY {
    ...
    //
    UCHAR PersistentReservationKey[8];
    UCHAR PRKeyValid;
...
typedef struct _DSM_FAILOVER_GROUP {
    ...
    //
    // Current amount of sequential IO that has gone on on this path.
    //
    ULONGLONG SequentialIO;
...
typedef struct _DSM_TARGET_PORT_GROUP_ENTRY {
    ...
    //
    // Flag to indicate if this has been returned in any subsequent RPTG after
    // it is initially built. (If this flag is not set after parsing the RPTG
...
typedef struct _DSM_CONTROLLER_LIST_ENTRY {
    ...
    // Identifier.
    //
    __field_ecount(IdLength) PUCHAR Identifier;
