...
#ifdef ALLOC_PRAGMA
    #pragma alloc_text(PAGE, DsmpBuildDeviceNameLegacy)
...
--*/
#pragma warning (suppress:28195) // when Block is not null on return, the function allocates memory.
{
    PVOID Block = NULL;
...
#pragma warning (suppress:6014) // This is an allocator. Block gets freed on calling DsmpFreePool
...
--*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        tempBuffer = DsmpAllocatePool(NonPagedPool, inquiryStrings.MaximumLength, DSM_TAG_REG_VALUE_RELATED);
...
--*/
{
    ...
    tempStrings.Buffer = DsmpAllocatePool(NonPagedPool, Context->SupportedDevices.MaximumLength, DSM_TAG_REG_VALUE_RELATED);
    ...
    if (DeviceIdType == DSM_DEVID_SERIAL_NUMBER) {
        ...
        if (codeSet == StorageIdCodeSetBinary) {
            ...
            if (identifierSize) {
                ...
                //
                buffer = DsmpAllocatePool(NonPagedPool, identifierSize + 1, DSM_TAG_DEV_ID);
                DSM_ASSERT(buffer);
        ...
        if (identifierSize) {
            ...
            buffer = DsmpAllocatePool(NonPagedPool, identifierSize, DSM_TAG_DEV_ID);
...
--*/
{
    ...
    // Allocate the buffer.
    //
    buffer = DsmpAllocatePool(NonPagedPool,
                              actualLength,
    ...
    for (i = 0, j = 0; i < Length; i++) {
        ...
        //
        // If it's 0x0F or less, just convert the entire byte.
        //
        if (HexBuffer[i] < 0x10) {
...
--*/
{
    ...
    if ((passThrough->ScsiPassThrough.ScsiStatus == SCSISTAT_GOOD) &&
        ...
        //
        serialNumber = DsmpAllocatePool(NonPagedPool,
        ...
        if (serialNumber) {
            ...
            // Convert to a more ascii-ish format so that other routines don't puke.
            //
...
__Retry_ModeSense:
    ...
    cdb->MODE_SENSE.Reserved3 = 0x01;   // SubpageCode
...
--*/
{
    ...
    hardwareId = DsmpAllocatePool(NonPagedPool, sizeNeeded, DSM_TAG_DEV_HARDWARE_ID);
    if (!hardwareId) {
...
--*/
{
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                TRACE_FLAG_PNP,
                "DsmpBuildDeviceNameLegacy (DevInfo %p): Entering function.\n",
                DeviceInfo));
    ...
    if (sizeNeeded > 0) {
        ...
        deviceName = DsmpAllocatePool(NonPagedPool, sizeNeeded, DSM_TAG_DEV_NAME);
        if (!deviceName) {

            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to allocate memory for device name.\n",
                        DeviceInfo));
    ...
    if (!NT_SUCCESS(status)) {
        goto __Exit_DsmpBuildDeviceNameLegacy;
    }
    ...
    if (vendorIDLength) {
        ...
        unicodeString.Length = 0;
        unicodeString.MaximumLength = (USHORT) vendorIDLength;
        unicodeString.Buffer = copyOffSet;
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_PNP,
                    "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to convert vendor id to unicode string.\n",
                    DeviceInfo));
    ...
    *tmpPtr = WNULL;
    ...
    //
    // Append _ and set the copy offset to the end of the string.
    //
    RtlStringCbCatW(copyOffSet, sizeNeeded, L"_");
    copyOffSet += wcslen(copyOffSet);
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_PNP,
                    "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to convert product id to unicode string.\n",
                    DeviceInfo));
    ...
    *tmpPtr = WNULL;
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_PNP,
                    "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to convert serial number to unicode string.\n",
                    DeviceInfo));
    ...
    //
    // If there are spaces in the id, NULL at the first space.
    //
    tmpPtr = copyOffSet;
    while ((*tmpPtr) && (*tmpPtr != L' ')) {
        tmpPtr++;
    ...
    *tmpPtr = WNULL;
    ...
    TracePrint((TRACE_LEVEL_ERROR,
                TRACE_FLAG_PNP,
                "DsmpBuildDeviceNameLegacy (DevInfo %p): Device Name is %ws.\n",
                DeviceInfo,
                deviceName));

__Exit_DsmpBuildDeviceNameLegacy:
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                TRACE_FLAG_PNP,
                "DsmpBuildDeviceNameLegacy (DevInfo %p): Exiting function with deviceName %ws.\n",
                DeviceInfo,
                deviceName));
...
Routine Description:
    ...
    If the registry has the legacy device name built using page 0x80 info,
    updates the key name with the device name built using page 0x83 identifier.

Arguements:

    DeviceInfo          - Device instance
    DeviceNameLegacy    - Device name built using page 0x80 info.
    DeviceNameLegacyLen - Number of chars (including NULL) of the DeviceNameLegacy buffer.
    DeviceName          - Device name built using 0x83 identifier
...
--*/
{
    ...
    // The name of this key is the one built in DsmpBuildDeviceNameLegacy
    //
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                ASSERT(explicitSet.KeyValueInfo.DataLength == sizeof(UCHAR));
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    ASSERT(preferredPath.KeyValueInfo.DataLength == sizeof(ULONGLONG));
...
DsmpOpenDsmServicesParametersKey(
    __in IN  ACCESS_MASK AccessMask,
    __out OUT PHANDLE ParametersKey
    )
/*++
...
Routine Description:
    ...
    Helper routine to send down ReportTargetPortGroups request.
    Used if device supports ALUA.

    Note: This routine will allocate memory for the TPG info. It is
          the responsibility of the caller to free this buffer.
...
--*/
{
    ...
    ASSERT(TargetPortGroupsInfoLength && *TargetPortGroupsInfoLength == 0);
    ASSERT(TargetPortGroupsInfo && !(*TargetPortGroupsInfo));
...
__do_again:
    ...
    length = SPC3_TARGET_PORT_GROUPS_HEADER_SIZE + additionalLength;

    dataBuffer = DsmpAllocateAlignedPool(NonPagedPool,
                                         length,
                                         alignmentMask,
                                         DSM_TAG_SPT_DATA_BUFFER,
                                         &allocatedLength);
    if (!dataBuffer) {

        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_GENERAL,
                    "DsmpReportTargetPortGroups (DevObj %p): Failed to allocate mem for passthrough's databuffer.\n",
                    DeviceObject));
    ...
    // Build the cdb.
    //
    cdb = (PSPC3_CDB_REPORT_TARGET_PORT_GROUPS)passThrough.ScsiPassThroughDirect.Cdb;

    cdb->OperationCode = SPC3_SCSIOP_REPORT_TARGET_PORT_GROUPS;
    cdb->ServiceAction = SPC3_SERVICE_ACTION_TARGET_PORT_GROUPS;
    allocationLength = SPC3_TARGET_PORT_GROUPS_HEADER_SIZE + additionalLength;
    Get4ByteArrayFromUlong(allocationLength, cdb->AllocationLength);
    ...
    passThrough.ScsiPassThroughDirect.DataIn = 1;
    passThrough.ScsiPassThroughDirect.DataTransferLength = length;
    ...
    if ((passThrough.ScsiPassThroughDirect.ScsiStatus == SCSISTAT_GOOD) &&
        ...
        // Get the returned data.
        //
        GetUlongFrom4ByteArray(dataBuffer, returnedDataLength);

        if (additionalLength != returnedDataLength) {

            //
            // This must be the first pass where we only get the header back.
            // So send the request again, this time with the correct size.
            //
            DsmpFreePool(dataBuffer);
            dataBuffer = NULL;
            additionalLength = returnedDataLength;

            goto __do_again;
        }

        //
        // Allocate a buffer to hold just the TPG info.
        //
        *TargetPortGroupsInfo = DsmpAllocatePool(NonPagedPool,
                                                 SPC3_TARGET_PORT_GROUPS_HEADER_SIZE + additionalLength,
        ...
        if (*TargetPortGroupsInfo) {
            ...
            *TargetPortGroupsInfoLength = SPC3_TARGET_PORT_GROUPS_HEADER_SIZE + additionalLength;

        } else {

            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_GENERAL,
                        "DsmpReportTargetGroups (DevObj %p): Failed to allocate mem for TPG.\n",
                        DeviceObject));
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_GENERAL,
                    "DsmpReportTargetPortGroups (DevObj %p): NTStatus 0%x, ScsiStatus 0x%x.\n",
                    DeviceObject,
                    ioStatus.Status,
...
--*/
{
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                TRACE_FLAG_RW,
                "DsmpReportTargetPortGroupsAsync (DevObj %p): Entering function.\n",
    ...
    if (!irp) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_RW,
                    "DsmpReportTargetPortGroupsAsync (DevObj %p): Failed to allocate IRP.\n",
                    DeviceInfo->TargetObject));
    ...
    if (!mdl) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_RW,
                    "DsmpReportTargetPortGroupsAsync (DevObj %p): Failed to allocate MDL.\n",
                    DeviceInfo->TargetObject));
        goto __Exit_DsmpReportTargetPortGroupsAsync;
    }
    ...
    // Build the cdb.
    //
    cdb = (PSPC3_CDB_REPORT_TARGET_PORT_GROUPS)srb->Cdb;

    cdb->OperationCode = SPC3_SCSIOP_REPORT_TARGET_PORT_GROUPS;
    cdb->ServiceAction = SPC3_SERVICE_ACTION_TARGET_PORT_GROUPS;
    Get4ByteArrayFromUlong(TargetPortGroupsInfoLength, cdb->AllocationLength);

    srb->CdbLength = sizeof(SPC3_CDB_REPORT_TARGET_PORT_GROUPS);
    srb->TimeOutValue = SPC3_REPORT_TARGET_PORT_GROUPS_TIMEOUT;
    srb->Length = sizeof(SCSI_REQUEST_BLOCK);
    srb->Function = SRB_FUNCTION_EXECUTE_SCSI;
    srb->SenseInfoBuffer = tpgCompletionContext->SenseInfoBuffer;
    srb->SenseInfoBufferLength = SENSE_BUFFER_SIZE;
    srb->DataTransferLength = TargetPortGroupsInfoLength;
    srb->DataBuffer = TargetPortGroupsInfo;
    srb->ScsiStatus = srb->SrbStatus = 0;
    srb->NextSrb = 0;
    srb->SrbFlags = SRB_FLAGS_DONT_START_NEXT_PACKET | SRB_FLAGS_QUEUE_ACTION_ENABLE |
    ...
    //
    status = DsmSendRequest(((PDSM_CONTEXT)(DeviceInfo->DsmContext))->MPIOContext,
                            DeviceInfo->TargetObject,
                            irp,
                            (PVOID)DeviceInfo);
...
__Exit_DsmpReportTargetPortGroupsAsync:
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                TRACE_FLAG_RW,
                "DsmpReportTargetPortGroupsAsync (DevObj %p): Exiting function with status %x\n.",
                DeviceInfo->TargetObject,
                status));
    ...
    if (size < wcharsLeft) {
        ...
        if (NT_SUCCESS(RtlStringCchCopyNW(pathPtr, wcharsLeft, DSM_PATH, wcslen(DSM_PATH)))) {
            ...
            if (size < wcharsLeft) {
                ...
                RtlStringCchCatW(pathPtr, wcharsLeft / sizeof(WCHAR), L"#");
                wcharsLeft -= sizeof(WCHAR);
                pathPtr++;
                ...
                //
                size = (sizeof(PVOID) + 1) * sizeof(WCHAR);
                if (size <= wcharsLeft) {
...
--*/
{
    ...
    *Descriptor = NULL;
    ...
    ioStatus = DsmpAllocatePool(NonPagedPool, sizeof(IO_STATUS_BLOCK), DSM_TAG_IO_STATUS_BLOCK);
    ...
    descriptor = DsmpAllocatePool(NonPagedPool, length, DSM_TAG_DEVICE_ID_LIST);
...
--*/
{
    ...
    ASSERT(TargetPortGroupsInfoLength && TargetPortGroupsInfo);
...
__Retry_Request:
    ...
    if ((passThrough.ScsiPassThroughDirect.ScsiStatus == SCSISTAT_GOOD) &&
        ...
        ASSERT(passThrough.ScsiPassThroughDirect.ScsiStatus == SCSISTAT_GOOD);
...
--*/
{
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                TRACE_FLAG_RW,
                "DsmpSetTargetPortGroupsAsync (DevObj %p): Entering function.\n",
                DeviceInfo->TargetObject));
    ...
    if (!irp) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_RW,
                    "DsmpSetTargetPortGroupsAsync (DevObj %p): Failed to allocate IRP.\n",
                    DeviceInfo->TargetObject));
    ...
    if (!mdl) {
        ...
        TracePrint((TRACE_LEVEL_ERROR,
                    TRACE_FLAG_RW,
                    "DsmpSetTargetPortGroupsAsync (DevObj %p): Failed to allocate MDL.\n",
                    DeviceInfo->TargetObject));
        goto __Exit_DsmpSetTargetPortGroupsAsync;
    }
    ...
    // Build the cdb.
    //
    cdb = (PSPC3_CDB_SET_TARGET_PORT_GROUPS)srb->Cdb;

    cdb->OperationCode = SPC3_SCSIOP_SET_TARGET_PORT_GROUPS;
    cdb->ServiceAction = SPC3_SERVICE_ACTION_TARGET_PORT_GROUPS;
    Get4ByteArrayFromUlong(TargetPortGroupsInfoLength, cdb->ParameterListLength);

    srb->CdbLength = sizeof(SPC3_CDB_SET_TARGET_PORT_GROUPS);
    srb->TimeOutValue = SPC3_SET_TARGET_PORT_GROUPS_TIMEOUT;
    srb->Length = sizeof(SCSI_REQUEST_BLOCK);
    srb->Function = SRB_FUNCTION_EXECUTE_SCSI;
    srb->SenseInfoBuffer = tpgCompletionContext->SenseInfoBuffer;
    srb->SenseInfoBufferLength = SENSE_BUFFER_SIZE;
...
__Exit_DsmpSetTargetPortGroupsAsync:
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                TRACE_FLAG_RW,
                "DsmpSetTargetPortGroupsAsync (DevObj %p): Exiting function with status %x.\n",
                DeviceInfo->TargetObject,
                status));
...
--*/
{
    ...
    lbSettings = DsmpAllocatePool(NonPagedPool,
                                  sizeNeeded,
                                  DSM_TAG_LB_POLICY);
                                    ...
                                    if (PreferredActiveDeviceInfo &&
                                        ...
                                        if (deviceInfo->DesiredState != DSM_DEV_ACTIVE_OPTIMIZED &&
                                            ...
                                            if (devState == DSM_DEV_ACTIVE_OPTIMIZED) {
                                                ...
                                                ASSERT(deviceInfo != activeDevice);
...
--*/
{
    ...
    dsmWorkItem = DsmpAllocatePool(NonPagedPool,
                                   sizeof(DSM_WORKITEM),
                                   DSM_TAG_WORKITEM);
    if (dsmWorkItem != NULL) {
...
--*/
{
    ...
    do {
        ...
        keyFullInfo = DsmpAllocatePool(NonPagedPoolCacheAligned, length, DSM_TAG_REG_KEY_RELATED);
    ...
    //
    valueBuf = DsmpAllocatePool(NonPagedPoolCacheAligned,
                                lengthOfValueName,
    ...
    //
    valueDataBuf = DsmpAllocatePool(NonPagedPoolCacheAligned,
                                    lengthOfValueData,
                                    DSM_TAG_REG_KEY_RELATED);
    ...
    for (index = 0; index < numValues && NT_SUCCESS(status); index++) {
        ...
        do {
            ...
            keyValueFullInfo = DsmpAllocatePool(NonPagedPoolCacheAligned, length, DSM_TAG_REG_KEY_RELATED);
    ...
    //
    subKey = DsmpAllocatePool(NonPagedPoolCacheAligned,
                              lengthOfKeyName,
                              DSM_TAG_REG_KEY_RELATED);
    ...
    for(index = 0; index < numSubKeys && NT_SUCCESS(status); index++) {
        ...
        do {
            ...
            keyBasicInfo = DsmpAllocatePool(NonPagedPoolCacheAligned,
                                            length,
                                            DSM_TAG_REG_KEY_RELATED);
...
--*/
{
    ...
    ASSERT(UseCacheForLeastBlocks);
    ASSERT(CacheSizeForLeastBlocks);
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            queryTable[0].Flags = RTL_QUERY_REGISTRY_DIRECT | RTL_QUERY_REGISTRY_REQUIRED;
            queryTable[0].Name = DSM_CACHE_SIZE_FOR_LEAST_BLOCKS;
            queryTable[0].EntryContext = CacheSizeForLeastBlocks;
            queryTable[0].DefaultType = REG_QWORD;
            queryTable[0].DefaultLength = sizeof(ULONGLONG);
