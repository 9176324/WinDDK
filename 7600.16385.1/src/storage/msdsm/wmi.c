...
--*/
{
    ...
    //
    WmiGlobalInfo->RegistryPath.Buffer = DsmpAllocatePool(NonPagedPool,
...
--*/
{
    ...
    // Allocate a buffer for the reg. path.
    //
    WmiInfo->RegistryPath.Buffer = DsmpAllocatePool(NonPagedPool,
                                                    RegistryPath->MaximumLength,
                                                    DSM_TAG_REG_PATH);
...
--*/
{
    ...
    *OutBufferSize = sizeNeeded;
    ...
    if (!DsmpIsSymmetricAccess(devInfo) &&
        ...
        rtpgDeviceInfo = DsmpGetActivePathToBeUsed(groupEntry);

        if (!rtpgDeviceInfo) {

            BOOLEAN sendTPG = FALSE;

            rtpgDeviceInfo = DsmpFindStandbyPathToActivateALUA(groupEntry, &sendTPG);
        }
    ...
    if (NT_SUCCESS(status) && rtpgDeviceInfo) {
        ...
        DsmpAdjustDeviceStatesALUA(groupEntry, NULL);
    }
    ...
    while (inx < DsmIds->Count) {
        ...
        if (!dsmPath->PrimaryPath &&
            !dsmPath->FailedPath) {
            ASSERT(groupEntry->LoadBalanceType != DSM_LB_ROUND_ROBIN &&
    ...
    *OutBufferSize = (DsmWmiVersion == DSM_WMI_VERSION_1) ? sizeof(DsmSetLoadBalancePolicy_OUT) : sizeof(DsmSetLoadBalancePolicyALUA_OUT);
        ...
        if (!DsmpIsDeviceFailedState(devInfo->State)) {
            ...
            if (dsmPath->PrimaryPath) {
                ...
                if (optimized) {
                    ...
                    // will require an SPTG to be sent down.
                    //
                    ...
                    if (loadBalancePolicy == DSM_LB_ROUND_ROBIN ||
                        ...
                        if (!groupEntry->PathToBeUsed) {
                            groupEntry->PathToBeUsed = devInfo->FailGroup;
                        }
                    }
                } else {
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (tempDevInfo->ALUASupport >= DSM_DEVINFO_ALUA_EXPLICIT && sendSTPG) {
            ...
            if (targetPortGroupsInfo) {
                ...
                ASSERT(jnx == numberDevInfoChanged);
                ASSERT(devInfoToUse);
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (devInfo != NULL) {
            ...
            // some other path.
            // The same argument holds for storages that support both implict and explicit
            // transitions. An explicit transition from us may have been overridden by an
            // implicit transition from the storage, leaving no paths in A/O state.
            //
            if (!DsmpIsSymmetricAccess(tempDevInfo) &&
                tempDevInfo->ALUASupport != DSM_DEVINFO_ALUA_EXPLICIT) {

                BOOLEAN sendTPG = FALSE;

                devInfo = DsmpFindStandbyPathToActivateALUA(groupEntry, &sendTPG);

                if (!devInfo) {

                    TracePrint((TRACE_LEVEL_WARNING,
                                TRACE_FLAG_PNP,
                                "DsmpSetLoadBalancePolicy (DsmIds %p): No FOG available for group %p\n",
                                DsmIds,
                                groupEntry));
                    ...
                    TracePrint((TRACE_LEVEL_WARNING,
                                TRACE_FLAG_PNP,
                                "DsmpSetLoadBalancePolicy (DsmIds %p): Using best alternative candidate %p.\n",
                                DsmIds,
                                devInfo));
                }
            } else {
...
--*/
{
    ...
    if (supportedLBPolicies->Version > DSM_WMI_VERSION) {
        ...
        ASSERT(supportedLBPolicies->Version == DSM_WMI_VERSION);
    }
...
Routine Description:
    ...
    Note: This routine MUST be called with DsmContextLock held
...
--*/
{
    ...
    tempBuffer = DsmpAllocatePool(NonPagedPool, DsmContext->SupportedDevices.MaximumLength, DSM_TAG_REG_VALUE_RELATED);
    ...
    for(index = 0; index < numSubKeys && NT_SUCCESS(status); index++) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            do {
                ...
                keyValueInfo = DsmpAllocatePool(NonPagedPoolCacheAligned, length, DSM_TAG_REG_KEY_RELATED);
                if (!keyValueInfo) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                ASSERT(keyValueInfo->DataLength == sizeof(ULONGLONG));
