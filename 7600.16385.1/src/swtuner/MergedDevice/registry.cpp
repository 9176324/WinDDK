...
// Routine Description:
//
//     Read a next subkey from the registry. 
//
// Arguments: 
//
//     RelativeTo     - starting point for the Path
//     Path           - path to the registry key
//     ParameterName  - name of the current Name 
//     NextParameterName  - name of the next name after current 
//     ParameterValue - converted value of the next Name
//
// Return Value:                                          
//                                                            
//     NTSTATUS
...
PptRegGetNextEntry(
    ...
    IN OUT   PUNICODE_STRING  NextParameterName,
    IN OUT PULONG ParameterValue
    )
{
    NTSTATUS                 Status;
    ...
    UNICODE_STRING           bipName;
    ULONG Size = 0;
    ULONG i = 0;

    if( ( NULL == Path ) || ( NULL == ParameterName ) || ( NULL == ParameterValue ) || ( NULL == NextParameterName )) {
        return STATUS_INVALID_PARAMETER;
    }
  ...
  if( !NT_SUCCESS( Status ) )
  {
    DbgPrint( "PptRegGetNextEntry failed to open Path Key.\n" );
    return Status;
  }

  Status = ZwQueryKey( RootRegHandle,
  				 	   KeyFullInformation,
  					   NULL,
  			  		   0,
  			  		   &Size );
  ...
  if( !NT_SUCCESS( Status ) )
  {
    DbgPrint( "PptRegGetNextEntry failed to get full information.\n" );
  }
  Size = min(Size, PAGE_SIZE) ;

  PKEY_FULL_INFORMATION fip = (PKEY_FULL_INFORMATION) ExAllocatePoolWithTag(PagedPool, Size, 'TadB');
  ...
  if( !NT_SUCCESS( Status) )
  {
    DbgPrint( "ZwQueryKey() failed. %x at %S\n", Status, Path->Buffer);
    ExFreePoolWithTag(fip,'TadB');
  ...
  while( i < fip->SubKeys ) //enumerate all subkeys
  {
    ZwEnumerateKey( RootRegHandle,
    			    i,
    			    KeyBasicInformation,
    			    NULL,
    			    0,
    			    &Size );
    bip = (PKEY_BASIC_INFORMATION) ExAllocatePoolWithTag(PagedPool, Size, 'TadB');
	...
	bipName.Length = (USHORT)bip->NameLength;
	bipName.MaximumLength = (USHORT)bip->NameLength;
	bipName.Buffer = bip->Name;
    RtlCopyUnicodeString(NextParameterName, &bipName);
    if (bFoundCurrent)
    {
        RtlUnicodeStringToInteger(NextParameterName, 10, ParameterValue);
        DbgPrint("Next Frequency Entry = %S\n",NextParameterName->Buffer);

        ExFreePoolWithTag(bip, 'TadB');
	...
	LONG lstatus = 1;
	lstatus = RtlCompareUnicodeString( &bipName,
    				 				  ParameterName,
    						 		  FALSE );
    ...
    ExFreePoolWithTag(bip,'TadB');
    Status = STATUS_UNSUCCESSFUL;
    if ((i == fip->SubKeys) && (bFoundCurrent == FALSE))
    {
        i=0; //set to start if nothing was found
        bFoundCurrent = TRUE;//and return the start position
    }
  }
