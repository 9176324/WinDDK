...
Exit:
    // m_hStartEvt no longer needed, so free resources
    if (NULL != m_hStartEvt)
    {
	CloseHandle(m_hStartEvt);
	m_hStartEvt = NULL;
    }
...
Synchronization: 
    Is implemented in CTask::End() method.
 
Arguments:
    dwDelay - amount of time to wait for thread to exit
...
/*++
Routine Description:
    Method is called for each message received by the message pump
    created by this thread.  The class that inherits from CMessageTask
    should always override this virtual method.

Synchronization: 
    None
 
Arguments:
    msg - the message id of the Win32 message
