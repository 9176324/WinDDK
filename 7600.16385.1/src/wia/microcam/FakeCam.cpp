...
HRESULT WiaMCamGetDeviceInfo(MCAM_DEVICE_INFO *pDeviceInfo, MCAM_ITEM_INFO **ppItemList)
{
    ...
    *ppItemList = NULL;
    ...
    // NOTICE-8/4/2000-davepar Put properties into an INI file
...
HRESULT WiaMCamReadEvent(MCAM_DEVICE_INFO *pDeviceInfo, MCAM_EVENT_INFO **ppEventList)
{
    ...
    UNREFERENCED_PARAMETER(ppEventList);
...
HRESULT SearchDir(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, __in PTSTR ptszPath)
{
    ...
    //
    // NOTICE-10/18/2000-davepar Do the same for other image types
    //
...
#pragma prefast(suppress:__WARNING_MEMORY_LEAK, "freed by WiaMCamFreeItemInfo().")
                        ...
                        WIN32_FIND_DATA *pFindData)
{
    ...
    hr = wiauStrT2W(ptszShortName, pItem->pwszName, iSize * sizeof(WCHAR));
...
HRESULT ReadDimFromJpeg(__in PTSTR ptszFullName, WORD *pWidth, WORD *pHeight)
{
    ...
    *pHeight = 0;
    ...
    pBufferEnd = &pBuffer[BytesToRead];
    ...
    // Ensure that we've got at least two bytes read so that accessing
    // pCur[1] and pCur[0] won't go past the end of the buffer.
    while (SUCCEEDED(hr) &&
           BytesRead >= 2 &&
           pCur[1] != 0xc0)
    {
        ...
        {
            // Make sure that the ByteSwapWord() below does not go past the end
            // of the buffer.
            if (pCur + 2 + sizeof(WORD) - 1 >= pBufferEnd)
            {
                hr = E_FAIL;
                REQUIRE_SUCCESS(hr, "ReadDimFromJpeg", "Not a JFIF format image");
            }
    ...
    // Make sure that the ByteSwapWord()'s below does not go past the end
    // of the buffer.
    if (pCur + 7 + sizeof(WORD) - 1 >= pBufferEnd)
    {
        hr = E_FAIL;
        REQUIRE_SUCCESS(hr, "ReadDimFromJpeg", "Not a JFIF format image");
    }
