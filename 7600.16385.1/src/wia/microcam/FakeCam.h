...
HRESULT CreateFolder(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppFolder, __in PTSTR ptszFullName);
HRESULT CreateImage(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppImage, __in PTSTR ptszFullName);
HRESULT CreateNonImage(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppNonImage, __in PTSTR ptszFullName);
HRESULT SetCommonFields(MCAM_ITEM_INFO *pItem, __in PTSTR ptszShortName, __in PTSTR ptszFullName, WIN32_FIND_DATA *pFindData);
...
//
inline HRESULT MakeFullName(PTSTR __out_ecount(cchFullNameSize) ptszFullName, UINT cchFullNameSize, __in PTSTR ptszPath, __in PTSTR ptszFileName)
{
    return StringCchPrintf(ptszFullName, cchFullNameSize, _T("%s\\%s"), ptszPath, ptszFileName);
}
...
HRESULT ReadJpegHdr(__in PTSTR ptszFileName, BYTE **ppBuf);
