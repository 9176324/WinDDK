                                    ...
                                    LPDWORD                 pcbActualData )
{
    ...
    UNREFERENCED_PARAMETER(pcbActualData);
...
{
    UNREFERENCED_PARAMETER(lpBuffer);
    UNREFERENCED_PARAMETER(nNumberOfBytes);
    UNREFERENCED_PARAMETER(lpOverlapped);
                                        ...
                                        HKEY hParametersKey )
{
    ...
    UNREFERENCED_PARAMETER(hParametersKey);
    ...
    *szDeviceNameW = L'\0';
    ...
    if ( StrCmpI("AUTO", m_pszDeviceNameA) ){
        ...
        WCHAR *pTempFile = (WCHAR*) LocalAlloc(LPTR, sizeof(WCHAR)*MAX_PATH);

        if(pTempFile)
        {
            ActualLength = GetTempPathW(MAX_PATH,pTempFile);
            ...
            if (SUCCEEDED(hres))
            {
                SHCreateDirectory(NULL,pTempFile);

                if (!GetTempFileNameW(pTempFile, L"StiTestDrvFile", 0, pTempFile))
                {
            ...
            if (SUCCEEDED(hres))
            {
                m_DeviceDataHandle = CreateFileW( pTempFile,
                                                GENERIC_READ | GENERIC_WRITE ,                     // Access mask
                                                0,    
                                                NULL,                              // SA
                                                CREATE_ALWAYS,                     // Create disposition
...
{
    UNREFERENCED_PARAMETER(fRefresh);
