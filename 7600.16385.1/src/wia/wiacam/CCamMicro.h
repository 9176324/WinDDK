...
typedef HRESULT (__stdcall *FPOpen)(MCAM_DEVICE_INFO *pDeviceInfo, __in PWSTR pwszPortName);
...
public:
    ...
    HRESULT Init(__in PTSTR ptszMicroDriverName, MCAM_DEVICE_INFO **ppDeviceInfo);
    HRESULT UnInit(MCAM_DEVICE_INFO *pDeviceInfo);
    HRESULT Open(MCAM_DEVICE_INFO *pDeviceInfo, __in PWSTR pwszPortName);
