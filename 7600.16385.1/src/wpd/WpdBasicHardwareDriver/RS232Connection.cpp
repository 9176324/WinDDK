...
RS232Connection::RS232Connection()
{
    ...
    //COMMTIMEOUTS m_TimeoutsDefault = { 0x01, 0, 0, 0, 0 };
    // Below fails--continues to truncate last byte
    //COMMTIMEOUTS m_TimeoutsDefault = { MAXDWORD, 0, 0, 0, 0 };
    // Below value (25) appears to interfere with sensor's
    // ability to read new intervals sent by the driver.
    //COMMTIMEOUTS m_TimeoutsDefault = { 25, 0, 0, 0, 0 };
    // Below value (0) somehow prohibits the driver-side
    // event from ever firing.
    //COMMTIMEOUTS m_TimeoutsDefault = { 0, 0, 0, 0, 0 };

    // After making a change to the firmware (and moving the
    // code that assigned the interval value), the following
    // default timeout worked and was tested down to 50ms.
    //COMMTIMEOUTS m_TimeoutsDefault = { 40, 0, 0, 0, 0 };

    //However, once we began testing with the Sensiron temp
    //and humidity sensor, it was necessary to increase
    //the ReadIntervalTimeout value by 20 milliseconds.
    // COMMTIMEOUTS m_TimeoutsDefault = { 60, 0, 0, 0, 0 };

    //And, once we began testing with the Memsic dual-axis
    //accelerometer, it was necessary to increase
    //the ReadIntervalTimeout value by 60 milliseconds.
    COMMTIMEOUTS m_TimeoutsDefault = { 120, 0, 0, 0, 0 };

    // After making a change to the firmware (and moving the
    // code that assigned the interval value), the following
    // default timeout failed to work--the firmware was still
    // missing the Interval prop.
    //COMMTIMEOUTS m_TimeoutsDefault = { 25, 0, 0, 0, 0 };
...
-----------------------------------------------------------------------------*/
HRESULT RS232Connection::Connect(LPCWSTR wszPortName, HANDLE *phCommPort)
{
    HRESULT hr          = S_OK;
    DWORD   dwLastError = 0;
