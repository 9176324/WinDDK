...
const GUID* g_SupportedContactFormats[] =
{
    ...
    __out   IPortableDevicePropVariantCollection* pMethods)
{
    ...
    {
        if (Format == (*g_SupportedContactFormats[i]))
        {
    ...
    __out   IPortableDevicePropVariantCollection* pFormats)
{
    ...
    if (hr == S_OK)
    {
        ...
        for (DWORD i=0; i<ARRAYSIZE(g_SupportedContactFormats); i++)
        {
            PROPVARIANT pv = {0};
            pv.vt = VT_CLSID;
            pv.puuid = (CLSID*)g_SupportedContactFormats[i]; // assignment, do not call PropVariantClear
    ...
    __out   IPortableDeviceValues* pAttributes)
{
    ...
    if (Format == WPD_OBJECT_FORMAT_ABSTRACT_CONTACT)
    {
        hr = pAttributes->SetStringValue(WPD_FORMAT_ATTRIBUTE_NAME, L"AbstractContactFormat");
        CHECK_HR(hr, "Failed to set WPD_FORMAT_ATTRIBUTE_NAME");
        // Has no MIME type
    }
    else if (Format == WPD_OBJECT_FORMAT_VCARD2)
    {
        hr = pAttributes->SetStringValue(WPD_FORMAT_ATTRIBUTE_NAME, L"VCard2Format");
        CHECK_HR(hr, "Failed to set WPD_FORMAT_ATTRIBUTE_NAME");
        if (hr == S_OK)
        {
            // Has no MIME type, set as empty string
            hr = pAttributes->SetStringValue(WPD_FORMAT_ATTRIBUTE_MIMETYPE, L"text/x-vcard");
            CHECK_HR(hr, "Failed to set WPD_FORMAT_ATTRIBUTE_MIMETYPE");
        }
    }
    else
    {
