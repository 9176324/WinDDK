    ...
    __out   IPortableDevicePropVariantCollection* pFormats)
{
    ...
    {
        pv.puuid = (CLSID*)&WPD_OBJECT_FORMAT_ABSTRACT_CONTACT;
        hr = pFormats->Add(&pv);
        CHECK_HR(hr, "Failed to add WPD_OBJECT_FORMAT_ABSTRACT_CONTACT");

        pv.puuid = (CLSID*)&WPD_OBJECT_FORMAT_VCARD2;
        hr = pFormats->Add(&pv);
        CHECK_HR(hr, "Failed to add WPD_OBJECT_FORMAT_VCARD2");
    }
