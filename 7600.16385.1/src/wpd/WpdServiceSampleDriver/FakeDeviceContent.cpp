...
#pragma prefast(suppress: __WARNING_MEMORY_LEAK, "pContactsService does not leak memory because ~FakeContent() will release it")
    FakeContactsServiceContent* pContactsService = new FakeContactsServiceContent();
    if (pContactsService)
    {
...
#pragma prefast(suppress: __WARNING_MEMORY_LEAK, "pFakeStorage does not leak memory because ~FakeContent() will release it")
    FakeStorage* pFakeStorage = new FakeStorage();
    if (pFakeStorage)
    {
