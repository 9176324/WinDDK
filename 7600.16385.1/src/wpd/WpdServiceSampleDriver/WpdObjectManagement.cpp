...
HRESULT WpdObjectManagement::Initialize(__in    FakeDevice* pDevice)
{
    if (pDevice == NULL)
    {
        return E_POINTER;
    ...
    __out   IPortableDeviceValues*  pResults)
{
    ...
    if (hr == S_OK)
    {
        ...
        hr = m_pDevice->CreatePropertiesOnlyObject(Scope, pObjectProperties, &pszObjectID, pEventParams);
