...
HRESULT WpdObjectPropertiesBulk::Initialize(__in FakeDevice *pDevice)
{
    ...
    __out   IPortableDeviceValues*  pResults)
{
    ...
    if (hr == S_OK)
    {
        for (DWORD dwIndex = pContext->NextObject, dwCount = 0; dwCount < cObjects; dwCount++, dwIndex++)
        {
            ...
            if (hr == S_OK)
            {
                // If a key list was supplied, get the specified object properties, otherwise get all
                // properties.
