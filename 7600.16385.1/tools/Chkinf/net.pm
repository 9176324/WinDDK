    ...
    foreach $line (@lines) {
        ...
        if ($LineItems[2] =~ /SERVICE/i)
        {
            ...
            if ($LineItems[1] =~ /PARAMS/i)
            {
                ...
                # Make sure inbox driver has flow control disabled
                if ($NET::Internal)
                {
                    if ($LineItems[1] =~ /PARAMS\\[^,]*FlowControl/i)
                    {
                        my($str);                
                        
                        $str = $LineItems[4];
                        
                        # remove quota at the begining and end of the string
                        while($str =~ /^\"/)
                        {
                            $str =~ s/^\"//;
                        }
                        ...
                        if ((uc($LineItems[2]) eq "DEFAULT") && !($str eq "0"))
                        {
                            AddNetCardError(4211,$inffile->getIndex($Section, $count));                    
                        }
                    }
                }
            }
