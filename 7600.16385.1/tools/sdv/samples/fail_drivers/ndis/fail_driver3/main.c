    ...
    )
{
    ...
    do
    {
        ...
        // ------------------------------------------------------------------------------------
        // SDV - Injected Defect - Rule: Init_RegisterInterrupt
        // Note: Commenting the line below , we don't save the state into the Adpater context
        //        and the routine for de-registering the interrupts won't be called.
        // ------------------------------------------------------------------------------------ 
    ...
    )
{
    ...
    // ------------------------------------------------------------------------------------
    // SDV - Injected Defect - Rule: SpinLock
    // Note: Commenting the line below, the SpinLock is not released when exiting
    //         the MiniportPause method. 
    // Note: SpinLockDpr will check for the case where 
    //          NdisDprAcquireSpinLock and NdisDprReleaseSpinLock are used instead.
    // ------------------------------------------------------------------------------------ 
    ...
    )
{
    ...
    if (pAdapter->LastRefCount <= pAdapter->RefCount)
    {
        ...
        // ------------------------------------------------------------------------------------
        // SDV - Injected Defect - Rule: SpinLockRelease
        // Note: Inserting the line below, the SpinLock is released twice when exiting
        //         the MiniportCheckForHang method 
        // Note: SpinLockDprRelease will check for the case where 
        //          NdisDprAcquireSpinLock and NdisDprReleaseSpinLock are used instead
        // ------------------------------------------------------------------------------------ 
        NdisReleaseSpinLock(&pAdapter->Lock);
        // ------------------------------------------------------------------------------------
    ...
    )
{
    ...
    // ------------------------------------------------------------------------------------
    // SDV - Injected Defect - Rule: Irql_Miniport_Driver_Function
    // Note: In the sample below, the IRQL is raised before calling 
    //        NdisMDeregisterMiniportDriver, which should be called only at PASSIVE_LEVEL.
    // ------------------------------------------------------------------------------------ 
